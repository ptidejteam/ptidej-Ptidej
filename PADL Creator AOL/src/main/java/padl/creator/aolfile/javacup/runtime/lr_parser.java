...
public abstract class lr_parser {
			...
			/* if its less than zero, then it encodes a reduce action */
				...
				act =
					this.get_reduce(
						((Symbol) this.stack.peek()).parse_state,
						lhs_sym_num);
				this.debug_message("# Reduce rule: top state "
						+ ((Symbol) this.stack.peek()).parse_state
	...
	public void debug_reduce(
		...
		final int nt_num,
		final int rhs_size) {
		this.debug_message("# Reduce with prod #" + prod_num + " [NT=" + nt_num
				+ ", " + "SZ=" + rhs_size + "]");
	}
	...
	public void debug_shift(final Symbol shift_tkn) {
		this.debug_message("# Shift under term #" + shift_tkn.sym
	...
	public void dump_stack() {
		...
		/* dump the stack */
		for (int i = 0; i < this.stack.size(); i++) {
			this.debug_message("Symbol: "
					+ ((Symbol) this.stack.elementAt(i)).sym + " State: "
					+ ((Symbol) this.stack.elementAt(i)).parse_state);
		}
		this.debug_message("==========================================");
	}
	 ...
	 */
	protected boolean error_recovery(final boolean debug)
		...
		for (;;) {
			...
			if (debug) {
				this.debug_message("# Consuming Symbol #"
						+ this.lookahead[0].sym);
			}
	...
	protected boolean find_recovery_config(final boolean debug) {
		...
		final int right_pos = ((Symbol) this.stack.peek()).right;
		...
		while (!this.shift_under_error()) {
			/* pop the stack */
			if (debug) {
				this.debug_message("# Pop stack by one, state was # "
						+ ((Symbol) this.stack.peek()).parse_state);
			}
		...
		act =
			this.get_action(((Symbol) this.stack.peek()).parse_state, this
				.error_sym());
		if (debug) {
			this.debug_message("# Recover state found (#"
					+ ((Symbol) this.stack.peek()).parse_state + ")");
			this.debug_message("# Shifting on error to state #" + (act - 1));
		}

		/* build and shift a special error Symbol */
		error_token = new Symbol(this.error_sym(), left_pos, right_pos);
	 ...
	 * @param debug should we produce debugging messages as we parse.
	 */
	protected void parse_lookahead(final boolean debug)
			throws java.lang.Exception {
		/* the current action code */
		int act;
		...
		if (debug) {
			...
			this.debug_message("# Current Symbol is #"
					+ this.cur_err_token().sym);
			this.debug_message("# Current state is #"
					+ ((Symbol) this.stack.peek()).parse_state);
		}
	...
	protected boolean shift_under_error() {
		...
		return this.get_action(((Symbol) this.stack.peek()).parse_state, this
			.error_sym()) > 0;
	}
	 ...
	 * @param debug should we produce debugging messages as we parse.
	 */
	protected boolean try_parse_ahead(final boolean debug)
			throws java.lang.Exception {
		int act;
		short lhs, rhs_size;
		...
		for (;;) {
			...
			if (act > 0) {
				...
				if (debug) {
					this.debug_message("# Parse-ahead shifts Symbol #"
							+ this.cur_err_token().sym + " into state #"
							+ (act - 1));
				}
			...
			/* < 0 encodes a reduce */
				...
				if (debug) {
					this.debug_message("# Parse-ahead reduces: handle size = "
