...
public class lalr_state {
	...
	public static Enumeration all() {
		return lalr_state._all.elements();
	}
	...
	public static lalr_state build_machine(final production start_prod)
		...
		final Stack work_stack = new Stack();
		...
		lalr_state._all_kernels.put(kernel, start_state);
		...
		while (!work_stack.empty()) {
			...
			for (s = outgoing.all(); s.hasMoreElements();) {
				...
				new_st = (lalr_state) lalr_state._all_kernels.get(kernel);
				...
				if (new_st == null) {
					...
					lalr_state._all_kernels.put(kernel, new_st);
				}
	...
	/** Helper routine for debugging -- produces a dump of the given state
	  * onto System.out.
	  */
	protected static void dump_state(final lalr_state st) throws internal_error {
		lalr_item_set itms;
		lalr_item itm;
		...
		itms = st.items();
		for (final Enumeration e = itms.all(); e.hasMoreElements();) {
	 ...
	 */
	public static lalr_state find_state(final lalr_item_set itms) {
		if (itms == null) {
			return null;
		}
		else {
			return (lalr_state) lalr_state._all.get(itms);
		}
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public lalr_state(final lalr_item_set itms) throws internal_error {
		...
		this._index = lalr_state.next_index++;

		/* store the items */
		this._items = itms;

		/* add to the global collection, keyed with its item set */
		lalr_state._all.put(this._items, this);
	}
	 ...
	 */
	public void add_transition(final symbol on_sym, final lalr_state to_st)
			throws internal_error {
		lalr_transition trans;

		/* create a new transition object and put it in our list */
		trans = new lalr_transition(on_sym, to_st, this._transitions);
	...
	public void build_table_entries(
		final parse_action_table act_table,
		final parse_reduce_table reduce_table) throws internal_error {
		parse_action_row our_act_row;
		parse_reduce_row our_red_row;
		lalr_item itm;
		parse_action act, other_act;
		symbol sym;
		final terminal_set conflict_set = new terminal_set();

		/* pull out our rows from the tables */
		our_act_row = act_table.under_state[this.index()];
		our_red_row = reduce_table.under_state[this.index()];

		/* consider each item in our state */
		for (final Enumeration i = this.items().all(); i.hasMoreElements();) {
			itm = (lalr_item) i.nextElement();
		...
		for (lalr_transition trans = this.transitions(); trans != null; trans =
			trans.next()) {
			...
			if (!sym.is_non_term()) {
				...
				if (our_act_row.under_term[sym.index()].kind() == parse_action.ERROR) {
					...
					/* we now have at least one conflict */
					final production p =
						((reduce_action) our_act_row.under_term[sym.index()])
							.reduce_with();
	...
	/** Equality comparison. */
	public boolean equals(final lalr_state other) {
	...
	protected boolean fix_with_precedence(
		final production p,
		final int term_index,
		final parse_action_row table_row,
		final parse_action act)

	throws internal_error {

		final terminal term = terminal.find(term_index);
		...
		if (p.precedence_num() > assoc.no_prec) {
			...
			if (p.precedence_num() > term.precedence_num()) {
				table_row.under_term[term_index] =
					this.insert_reduce(table_row.under_term[term_index], act);
				return true;
			}
			...
			/* if terminal precedes rule, put shift in table */
				...
				table_row.under_term[term_index] =
					this.insert_shift(table_row.under_term[term_index], act);
				return true;
			}
	...
	*/
	protected parse_action insert_action(
		final parse_action a1,
		final parse_action a2,
		final int act_type) throws internal_error {
	...
	protected parse_action insert_reduce(
		final parse_action a1,
		final parse_action a2) throws internal_error {
		return this.insert_action(a1, a2, parse_action.REDUCE);
	}
	...
	protected parse_action insert_shift(
		final parse_action a1,
		final parse_action a2) throws internal_error {
		return this.insert_action(a1, a2, parse_action.SHIFT);
	}
	...
	protected void propagate_lookaheads() throws internal_error {
		...
		for (final Enumeration itm = this.items().all(); itm.hasMoreElements();) {
	...
	protected void report_conflicts(final terminal_set conflict_set)
			throws internal_error {
		lalr_item itm, compare;
		final symbol shift_sym;

		boolean after_itm;

		/* consider each element */
		for (final Enumeration itms = this.items().all(); itms
			.hasMoreElements();) {
			...
			if (itm.dot_at_end()) {
				...
				for (final Enumeration comps = this.items().all(); comps
					.hasMoreElements();) {
	...
	protected void report_reduce_reduce(
		final lalr_item itm1,
		final lalr_item itm2) throws internal_error {
	...
	public String toString() {
		...
		/* dump the item set */
		result = "lalr_state [" + this.index() + "]: " + this._items + "\n";

		/* do the transitions */
		for (tr = this.transitions(); tr != null; tr = tr.next()) {
			result += tr;
			result += "\n";
		}
