...
abstract class AbstractClassFileCreator {
	private static String[] computeMethodCallees(
		...
		if (codeAttributeInfo != null) {
			...
			final ImmutableCodeSegment immutableCodeSegment = new ImmutableCodeSegment(
					rawCode);
			final int numberOfInstructions = immutableCodeSegment
					.getNumInstructions();
			...
			for (int j = 0; j < numberOfInstructions; j++) {
				...
				case JVMConstants.INVOKEVIRTUAL: {
					int num = ByteArray.getShortAtOffset(rawCode, offset + 1);
					ConstantPoolEntry constantPoolEntry = aConstantPool
							.get(num);
					...
					if (indices != null) {
						String className = aConstantPool.getAsJava(indices[0]);
						String nameAndType = aConstantPool
								.getAsJava(indices[1]);
						calles.add(className + '.' + nameAndType.substring(0,
								nameAndType.indexOf('.')));
					}
				}
					break;
	...
	public AbstractClassFileCreator(final String[] someClassFiles,
		...
		for (int i = 0; i < someClassFiles.length; i++) {
			...
			if (someClassFiles[i].endsWith(".jar")) {
				classFiles = SubtypeLoader.loadSubtypesFromJar(null,
						someClassFiles[i], ".class");
			}
			else if (someClassFiles[i].endsWith(".class")) {
				classFiles = SubtypeLoader.loadSubtypeFromFile(null,
						someClassFiles[i], ".class");
			}
				...
				classFiles = SubtypeLoader.loadRecursivelySubtypesFromDir(null,
						someClassFiles[i], ".class");
			}
			else {
				classFiles = SubtypeLoader.loadSubtypesFromDir(null,
						someClassFiles[i], ".class");
			}
	...
	public AbstractClassFileCreator(final JarInputStream inputStream,
			final boolean storeMethodInvocation) {
		...
		classFiles = SubtypeLoader.loadSubtypesFromJarInputStream(null,
				inputStream, ".class");
		...
		for (int j = 0; j < classFiles.length; j++) {
			...
			int index = 0;
			while (index < numberOfClassFiles
					&& ((ClassFile) this.listOfClassFiles.get(index)).getName()
							.compareTo(currentClassFileName) <= 0) {
	...
	public void create(final ICodeLevelModel aCodeLevelModel) {
		final ClassFile[] listOfEntities = this.constituentsRepository
				.getEntities();
		final ClassFile[] listOfElements = this.constituentsRepository
				.getElements();

		this.createPackages(aCodeLevelModel, this.listOfClassFiles,
				this.constituentsRepository, listOfEntities);

		this.createEntities(aCodeLevelModel, this.listOfClassFiles,
				this.constituentsRepository, listOfEntities);

		this.createElements(aCodeLevelModel, this.listOfClassFiles,
	...
	private void createElements(final ICodeLevelModel aCodeLevelModel,
		...
		for (int x = 0; x < listOfElements.length; x++) {
			...
			try {
				...
				buffer.append(constituentName
						.substring(constituentName.lastIndexOf('.') + 1));
				buffer.append("Priority");
				sortedElements[x] = ((Integer) this.getClass()
		...
		for (int elementRecogniserNumber = 0; elementRecogniserNumber < listOfElements.length; elementRecogniserNumber++) {
			...
			final Iterator iteratorOnEntities = listOfSourceConstituents
					.iterator();
			while (iteratorOnEntities.hasNext()) {
				...
				final ClassFile currentClass = (ClassFile) iteratorOnEntities
						.next();
				final char[] currentClassName = currentClass.getName()
						.toCharArray();

				if (!Utils.isAnonymousOrLocalEntity(currentClassName) && !Utils
						.isLocalOrLocalMemberEntity(currentClassName)) {

					final IFirstClassEntity firstClassEntity = Utils
							.getEntityOrCreateGhost(aCodeLevelModel,
					...
					if (!(firstClassEntity instanceof IGhost)) {
						...
						for (int i = 0; i < currentClass.getMethods()
								.length(); i++) {
							...
							final MethodInfo methodInfo = currentClass
									.getMethods().get(i);
							if (AttrUtils.getAttrByName(methodInfo.getAttrs(),
									"Synthetic") == null) {

								listOfSubmittedElements.add(
										new ExtendedMethodInfo(currentClass,
												methodInfo));
							}
						}

						for (int i = 0; i < currentClass.getFields()
								.length(); i++) {

							listOfSubmittedElements
									.add(new ExtendedFieldInfo(currentClass,
						...
						try {
							...
							//				this });
							buffer.append("recognize");
							buffer.append(constituentName.substring(
	...
	private void createEntities(final ICodeLevelModel aCodeLevelModel,
			final List listOfSourceConstituents,
			final ConstituentsRepository constituentsRepository,
			...
			if (!Utils.isAnonymousOrLocalEntity(entityID)
				...
				if (!Utils.isMemberEntity(entityID)) {
					...
					if (Utils.isClass(classFile)) {
						firstClassEntity = aCodeLevelModel.getFactory()
								.createClass(entityID,
										Utils.computeSimpleName(entityID));
					}
					else {
						firstClassEntity = aCodeLevelModel.getFactory()
								.createInterface(entityID,
										Utils.computeSimpleName(entityID));
					}
					firstClassEntity.setVisibility(classFile.getAccess());

					final IPackage enclosingPackage = Utils.getPackage(
							this.mapOfPackageNamesPackages, entityID);
					enclosingPackage.addConstituent(firstClassEntity);
					this.mapOfIDsEntities.put(entityDisplayID,
							firstClassEntity);
				}
				else {
					final IFirstClassEntity memberEntity;
					if (Utils.isClass(classFile)) {
						memberEntity = aCodeLevelModel.getFactory()
								.createMemberClass(entityID,
										Utils.computeSimpleName(entityID));
					}
					else {
						memberEntity = aCodeLevelModel.getFactory()
								.createMemberInterface(entityID,
										Utils.computeSimpleName(entityID));
					}
					memberEntity.setVisibility(classFile.getAccess());

					final char[] enclosingEntityID = ArrayUtils.subarray(
							entityID, 0, ArrayUtils.lastIndexOf(entityID, '$'));
		...
		while (iterator.hasNext()) {
			...
			if (!Utils.isAnonymousOrLocalEntity(entityName)
				...
				// IDs and entities.
				final IFirstClassEntity firstClassEntity = Utils
						.getEntityOrCreateGhost(aCodeLevelModel, entityName,
				...
				if (!(firstClassEntity
					...
					final String superClassDisplayName = classFile
							.getSuperName();
					final char[] superClassName = superClassDisplayName
							.toCharArray();
					final IFirstClassEntity superEntity = Utils
							.getEntityOrCreateGhost(aCodeLevelModel,
				...
				for (int i = 0; i < interfaceList.length(); i++) {
					final String superInterfaceDisplayName = interfaceList
							.get(i);
					final char[] superInterfaceName = superInterfaceDisplayName
							.toCharArray();
					final IFirstClassEntity superEntity = Utils
							.getEntityOrCreateGhost(aCodeLevelModel,
	...
	private void createPackages(final ICodeLevelModel aCodeLevelModel,
			final List listOfSourceConstituents,
			final ConstituentsRepository constituentsRepository,
			final ClassFile[] listOfEntities) {
		...
		while (iterator.hasNext()) {
			...
			final IPackage enclosingPackage = Utils.searchForPackage(
					aCodeLevelModel, entityName, new PackageCreator() {
			...
			if (!this.mapOfPackageNamesPackages.containsKey(packageName)) {
				this.mapOfPackageNamesPackages.put(String.valueOf(packageName),
						enclosingPackage);
			}
		}
	}

	public List recognizeAggregation(final List aListOfSubmittedConstituents,
	...
	public List recognizeAssociation(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
	...
	public List recognizeComposition(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
	...
	public List recognizeConstructor(final List aListOfSubmittedConstituents,
		...
		while (iterator.hasNext()) {
			...
			if (element instanceof ExtendedMethodInfo) {
				...
				if (Utils.isSpecialMethod(name)) {
					final IConstructor currentConstructor = aCodeLevelModel
							.getFactory().createConstructor(
									Utils.computeSignature(
											currentExtendedMethod),
									Util.computeSimpleName(currentExtendedMethod
											.getDeclaringClassName()));

					currentConstructor.setVisibility(
							currentExtendedMethod.getVisibility());

					final char[][] detectedParameters = currentExtendedMethod
							.getParameters();
	...
	public List recognizeContainerAggregation(
		...
		if (Utils.ENABLE_BUILT_IN_AAC_ANALYSIS) {
			ContainerRelationshipAnalyzer.recognizeContainerAggregation(
					aListOfSubmittedConstituents, aCodeLevelModel);
		}
	...
	public List recognizeCreation(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
	...
	public List recognizeDelegatingMethod(
		...
		while (iterator.hasNext()) {
			...
			if (member instanceof ExtendedMethodInfo) {
				final ExtendedMethodInfo extendedMethodInfo = (ExtendedMethodInfo) member;
				// final MethodInfo methodInfo = extendedMethodInfo.getMethod();

				final String[] callees = AbstractClassFileCreator
				...
				if (callees.length == 1) {
					if (callees[0].substring(callees[0].lastIndexOf('.') + 1)
							.equals(extendedMethodInfo.getName())) {

						final String calleesTypeName = callees[0].substring(0,
								callees[0].lastIndexOf('.'));

						final IFirstClassEntity currentEntity = Utils
								.getEntityOrCreateGhost(aCodeLevelModel,
										extendedMethodInfo
												.getDeclaringClassName(),
										this.mapOfIDsEntities);

						final Iterator iteratorOnContainerAggregation = currentEntity
								.getIteratorOnConstituents(
										IContainerAggregation.class);
						while (iteratorOnContainerAggregation.hasNext()) {
							final IContainerAggregation aggregation = (IContainerAggregation) iteratorOnContainerAggregation
									.next();
							...
							if (aggregation.getTargetEntity()
									.equals(calleesTypeName)) {

								// TODO: The target method should not be "null"! 
								final IDelegatingMethod delegatingMethod = aCodeLevelModel
										.getFactory().createDelegatingMethod(
	...
	public List recognizeEnum(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
	...
	public List recognizeField(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
	...
	public List recognizeGeneralisation(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
	...
	public List recognizeImplementation(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
	...
	public List recognizeMethod(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
		...
		while (iterator.hasNext()) {
			...
			if (element instanceof ExtendedMethodInfo) {
				...
				// of giving it the proper name and return type.
				final char[] name = currentExtendedMethod.getName();
				if (!Utils.isSpecialMethod(name) && !Utils.isSyntheticBridgeMethod(currentExtendedMethod)) {
					final IMethod currentMethod = aCodeLevelModel.getFactory()
							.createMethod(Utils.computeSignature(
									currentExtendedMethod), name);
					...
					currentMethod.setReturnType(
							currentExtendedMethod.getReturnType());

					currentMethod.setVisibility(
							currentExtendedMethod.getVisibility());
	...
	public List recognizeRelationship(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
		...
		RelationshipAnalyzer.recogniseRelationship(aListOfSubmittedConstituents,
				aCodeLevelModel, this.storeMethodInvocation);
	...
	public List recognizeSpecialisation(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
	...
	public List recognizeStructure(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
	...
	public List recognizeUnion(final List aListOfSubmittedConstituents,
			final ICodeLevelModel aCodeLevelModel) {
