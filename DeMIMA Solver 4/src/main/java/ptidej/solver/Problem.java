		...
		if (constraint != null) {
			...
			if (numberOfVar == 1) {
				...
				if (constraint instanceof GreaterOrEqualConstraint) {
					...
					args.add(this.findDynamicVariable((Variable) constraint
						.getVar(0)));
					args.add(Integer.valueOf(((LessOrEqualConstraint) constraint)
						.getConstant()));
				}
			}
			else {
			...
			if (constraint instanceof NotEqualConstraint) {
				args.add(Integer.valueOf(((NotEqualConstraint) constraint)
					.getConstant()));
			}
			if (constraint instanceof EqualConstraint) {
				args.add(Integer.valueOf(((EqualConstraint) constraint)
					.getConstant()));
			}

			args.add(Integer.valueOf(weight));
