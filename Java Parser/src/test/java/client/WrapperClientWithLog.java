...
public class WrapperClientWithLog extends WrapperClient {
    ...
    /**
     * @param javaFilePath Local absolute Java file path
    ...
    private String getUnifiedRelatvieJavaFileName(final String javaFilePath) {
        ...
        for (final NamedReader sourcepathEntry : this.javaProject.getSourcepathEntries()) {
            if (javaFilePath.contains(sourcepathEntry.getName())) {
                qualifiedClassName = javaFilePath
                        .replace(sourcepathEntry.getName() + Constants.SEPARATOR, Constants.EMPTY_STR)
                        .replace(Constants.SEPARATOR, Constants.SLASH);
                break;
            }
        }
    ...
    private void log(final String msg, final boolean display) {
        if (display) ProxyConsole.getInstance().debugOutput().println(msg);
    ...
    @Override
    public void parse(final NamedReader[] compilationUnitList, final ASTVisitor[] visitors) {
        ...
        this.displayAndLog(
                "Parsing " + this.javaFileNum + " java files took " + new Elapse(startTime, endTime) +
                        " with " + this.numOfErrors + " errors.");
        this.flush();
    }

    private void parseJavaFile(final NamedReader javaSource, final ASTVisitor[] visitors) {
        ...
        if (node instanceof CompilationUnit) {
            ...
            if (compilationUnit.getProblems().length > 0) {
                ...
                this.logProblems(compilationUnit.getProblems(), javaSource.getName());
            }
        }
        this.javaFileNum++;
