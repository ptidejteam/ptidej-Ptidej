...
public class SanityTest extends TestCase {
	...
	private static final String FILE = SanityTest.PATH + "CreatorJava.java";
	...
	protected void setUp() {
		if (SanityTest.CodeLevelModel == null) {
			try {
				SanityTest.CodeLevelModel = Factory.getInstance()
						.createCodeLevelModel("");
				SanityTest.CodeLevelModel.create(new JavaFileCreator(SanityTest.PATH,
						new String[] { SanityTest.FILE }));
			}
	...
	public void testA() {
		Assert.assertEquals("Number of classes", 1,
				SanityTest.CodeLevelModel.getNumberOfTopLevelEntities(IClass.class));
	}

	public void testB() {
		final IFirstClassEntity entity = SanityTest.CodeLevelModel
		...
		Assert.assertEquals("Class CreatorJava comment",
				SanityTest.TEST_B.replaceAll("\\r", "").replaceAll("\\n", ""),
				comment);
	}
	...
	public void testC() {
		...
		Assert.assertEquals("Constructor comment",
				SanityTest.TEST_C.replaceAll("\\r", "").replaceAll("\\n", ""),
				comment);
	}
}
