		...
		if (JavaSwingTest.FoundSolutions == null) {
			try {
				...
				final ICodeLevelModel codeLevelModel = Factory.getInstance()
						.createCodeLevelModel("Java Swing v1.4.2_11");
				codeLevelModel
						.create(new CompleteClassFileCreator(new String[] {
								"../EPI/target/test-classes/Java Swing v1.4.2_11/javax/swing/" }
						//, true
						));
				final IIdiomLevelModel idiomLevelModel = (IIdiomLevelModel) new AACRelationshipsAnalysis()
						.invoke(codeLevelModel);
				final String programString = StringBuilder
						.buildModelString(idiomLevelModel);
				final String motifString = StringBuilder
						.buildPatternString("Composite");
				final Hashtable domains = null;
				final EPISolver solver = new OptimisedBitVectorSolver(
						programString, motifString, "Composite",
						new Approximation(), domains);
