 ...
 * 	
 */
public class BitVectorOLD {// extends BitSet {
	...
	public void and(final BitVectorOLD set) {
	...
	public void andNot(final BitVectorOLD set) {
		this.bitset.removeAll(set.getBitSet());
	}

	// Removes the bit specified from the set
	public void clear(final int bit) {
		this.bitset.remove(Integer.valueOf(bit));
	}

	// Clone
	public Object clone() {
		final BitVectorOLD set = new BitVectorOLD();
		set.or(this);
		return set;
	}
	...
	public boolean equals(final Object obj) {
		...
		if (!(obj instanceof BitVectorOLD)) {
			return false;
		}
		else {
			final BitVectorOLD other = (BitVectorOLD) obj;
	...
	public boolean get(final int bit) {
		return this.bitset.contains(Integer.valueOf(bit));
	}
	...
	public void shift(final int offset, final int shiftSize) {
		final BitVectorOLD set = (BitVectorOLD) this.clone();
		...
		for (int index = 0; index < set.length(); index++) {
			...
			if (set.get(index)) {
				...
				this.bitset.add(Integer.valueOf(position));
			}
		}
	}
	...
	// with the bit set argument
	public void xor(final BitVectorOLD set) {
