...
public class lalr_item_set {
	 ...
	 */
	public lalr_item_set(final lalr_item_set other) throws internal_error {
		this.not_null(other);
	 ...
	 */
	public lalr_item add(final lalr_item itm) throws internal_error {
		lalr_item other;

		this.not_null(itm);

		/* see if an item with a matching core is already there */
		other = (lalr_item) this._all.get(itm);
	 ...
	 */
	public void add(final lalr_item_set other) throws internal_error {
		this.not_null(other);
		...
		while (consider.size() > 0) {
			...
			if (nt != null) {
				...
				for (p = nt.productions(); p.hasMoreElements();) {
					...
					new_itm =
						new lalr_item(prod, new terminal_set(new_lookaheads));

					/* add/merge item into the set */
					add_itm = this.add(new_itm);
	 ...
	 */
	public boolean contains(final lalr_item itm) {
	...
	/** Equality comparison. */
	public boolean equals(final lalr_item_set other) {
	...
	public boolean equals(final Object other) {
		if (!(other instanceof lalr_item_set)) {
			...
			return this.equals((lalr_item_set) other);
		}
	}
	 ...
	 */
	public lalr_item find(final lalr_item itm) {
	...
	public int hashCode() {
		...
		if (this.hashcode_cache == null) {
			...
			for (e = this.all(); e.hasMoreElements() /*&& cnt<5*/;) {
				result ^= ((lalr_item) e.nextElement()).hashCode();
			}

			this.hashcode_cache = Integer.valueOf(result);
		}
	 ...
	 */
	public boolean is_subset_of(final lalr_item_set other)
	 ...
	 * @param other the other set in question.
	 */
	public boolean is_superset_of(final lalr_item_set other)
			throws internal_error {
		this.not_null(other);
	 ...
	 */
	public void remove(final lalr_item itm) throws internal_error {
		this.not_null(itm);
	 ...
	 */
	public void remove(final lalr_item_set other) throws internal_error {
		this.not_null(other);
