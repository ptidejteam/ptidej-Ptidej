...
public class emit {
	...
	protected static void do_action_table(
		...
		final parse_action_table act_tab,
		final boolean compact_reduces) throws internal_error {
		...
		final short[][] action_table = new short[act_tab.num_states()][];
		...
		for (int i = 0; i < act_tab.num_states(); i++) {
			...
			final short[] temp_table = new short[2 * parse_action_row.size()];
			int nentries = 0;
			...
			for (int j = 0; j < parse_action_row.size(); j++) {
				...
				if (act.kind() != parse_action.ERROR) {
					...
					if (act.kind() == parse_action.SHIFT) {
						...
						temp_table[nentries++] =
							(short) (((shift_action) act).shift_to().index() + 1);
					}
		...
		emit.do_table_as_string(out, action_table);
		out.println(");");
		...
		emit.action_table_time = System.currentTimeMillis() - start_time;
	}

	// output an escape sequence for the given character code.
	protected static int do_escaped(final PrintWriter out, final char c) {
		final StringBuffer escape = new StringBuffer();
		if (c <= 0xFF) {
	...
	protected static int do_newline(
		final PrintWriter out,
		final int nchar,
		final int nbytes) {
		if (nbytes > 65500) {
	...
	protected static void do_reduce_table(
		final PrintWriter out,
		final parse_reduce_table red_tab) {
		lalr_state goto_st;
		final long start_time = System.currentTimeMillis();

		/* collect values for reduce-goto table */
		final short[][] reduce_goto_table = new short[red_tab.num_states()][];
		/* do each row of the reduce-goto table */
		for (int i = 0; i < red_tab.num_states(); i++) {
			/* make temporary table for the row. */
			final short[] temp_table = new short[2 * parse_reduce_row.size()];
			int nentries = 0;
		...
		out.print("    unpackFromStrings(");
		emit.do_table_as_string(out, reduce_goto_table);
		out.println(");");
		...
		emit.goto_table_time = System.currentTimeMillis() - start_time;
	}

	/*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/

	// print a string array encoding the given short[][] array.
	protected static void do_table_as_string(
		final PrintWriter out,
		final short[][] sa) {
		out.println("new String[] {");
		out.print("    \"");
		int nchar = 0, nbytes = 0;
		nbytes += emit.do_escaped(out, (char) (sa.length >> 16));
		nchar = emit.do_newline(out, nchar, nbytes);
		nbytes += emit.do_escaped(out, (char) (sa.length & 0xFFFF));
		nchar = emit.do_newline(out, nchar, nbytes);
		for (int i = 0; i < sa.length; i++) {
			nbytes += emit.do_escaped(out, (char) (sa[i].length >> 16));
			nchar = emit.do_newline(out, nchar, nbytes);
			nbytes += emit.do_escaped(out, (char) (sa[i].length & 0xFFFF));
			nchar = emit.do_newline(out, nchar, nbytes);
			for (int j = 0; j < sa[i].length; j++) {
				// contents of string are (value+2) to allow for common -1, 0 cases
				// (UTF-8 encoding is most efficient for 0<c<0x80)
				nbytes += emit.do_escaped(out, (char) (2 + sa[i][j]));
				nchar = emit.do_newline(out, nchar, nbytes);
			}
		}
		out.print("\" }");
	...
	protected static void emit_action_code(
		final PrintWriter out,
		final production start_prod) throws internal_error {
		production prod;
		...
		out.println("class " + emit.pre("actions") + " {");
		...
		out.println("  private final " + emit.parser_class_name + " parser;");

		/* constructor */
		out.println();
		out.println("  /** Constructor */");
		out.println("  " + emit.pre("actions") + "(" + emit.parser_class_name
				+ " parser) {");
		...
		out.println("  public final java_cup.runtime.Symbol "
				+ emit.pre("do_action") + "(");
		out.println("    int                        " + emit.pre("act_num,"));
		out.println("    java_cup.runtime.lr_parser " + emit.pre("parser,"));
		out.println("    java.util.Stack            " + emit.pre("stack,"));
		out.println("    int                        " + emit.pre("top)"));
		...
		out
			.println("      java_cup.runtime.Symbol " + emit.pre("result")
					+ ";");
		out.println();

		/* switch top */
		out.println("      /* select the action based on the action number */");
		out.println("      switch (" + emit.pre("act_num") + ")");
		out.println("        {");
		...
		for (final Enumeration p = production.all(); p.hasMoreElements();) {
			...
			for (int i = 0; i < prod.rhs_length(); i++) {
				...
				final symbol s = ((symbol_part) prod.rhs(i)).the_symbol();
				...
				final int index = prod.rhs_length() - i - 1; // last rhs is on top.
			...
			if (prod == start_prod) {
				out.println("          /* ACCEPT */");
				out.println("          " + emit.pre("parser")
						+ ".done_parsing();");
			}

			/* code to return lhs symbol */
			out.println("          return " + emit.pre("result") + ";");
			out.println();
		}
	 ...
	 * @param out stream to produce output on.
	 */
	protected static void emit_package(final PrintWriter out) {
		/* generate a package spec if we have a name for one */
		if (emit.package_name != null) {
			out.println("package " + emit.package_name + ";");
			out.println();
		}
	}
	 ...
	 * @param out stream to produce output on.
	 */
	protected static void emit_production_table(final PrintWriter out) {
		...
		// make short[][]
		final short[][] prod_table = new short[production.number()][2];
		...
		out.print("    unpackFromStrings(");
		emit.do_table_as_string(out, prod_table);
		out.println(");");
		...
		emit.production_table_time = System.currentTimeMillis() - start_time;
	}
	...
	public static void parser(
		...
		final int start_st,
		final production start_prod,
		final boolean compact_reduces,
		...
		out.println("//----------------------------------------------------");
		out.println();
		emit.emit_package(out);

		/* user supplied imports */
		for (int i = 0; i < emit.import_list.size(); i++) {
			out.println("import " + emit.import_list.elementAt(i) + ";");
		}
		...
		out.println("  */");
		out.println("public class " + emit.parser_class_name
				+ " extends java_cup.runtime.lr_parser {");

		/* constructors [CSA/davidm, 24-jul-99] */
		out.println();
		out.println("  /** Default constructor. */");
		out.println("  public " + emit.parser_class_name + "() {}");
		if (!suppress_scanner) {
			out.println();
		...
		emit.emit_production_table(out);
		emit.do_action_table(out, action_table, compact_reduces);
		emit.do_reduce_table(out, reduce_table);

		/* instance of the action encapsulation class */
		out.println("  /** Instance of action encapsulation class. */");
		out.println("  protected " + emit.pre("actions") + " action_obj;");
		out.println();
		...
		out.println("    {");
		out
			.println("      action_obj = new " + emit.pre("actions")
					+ "(this);");
		out.println("    }");
		out.println();
		...
		out.println("    return action_obj." + emit.pre("do_action(")
		...
		emit.emit_action_code(out, start_prod);

		emit.parser_time = System.currentTimeMillis() - start_time;
	}

	/** Build a string with the standard prefix. 
	 * @param str string to prefix.
	 */
	protected static String pre(final String str) {
		return emit.prefix + emit.parser_class_name + "$" + str;
	}
	protected static void set_lr_values(final boolean b) {
		emit._lr_values = b;
	}
	...
	public static void symbols(
		final PrintWriter out,
		final boolean emit_non_terms,
		final boolean sym_interface) {
		terminal term;
		non_terminal nt;
		final String class_or_interface = sym_interface ? "interface" : "class";
		...
		emit.symbols_time = System.currentTimeMillis() - start_time;
	}
