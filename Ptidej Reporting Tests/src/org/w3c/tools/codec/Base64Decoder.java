...
public class Base64Decoder
{
	...
	private void printHex(int x)
	{
		...
		int l = (x & 0x0f);
		System.out.print(
			(Character.valueOf((char) ((h > 9) ? 'A' + h - 10 : '0' + h)))
				.toString()
				+ (Character.valueOf((char) ((l > 9) ? 'A' + l - 10 : '0' + l)))
					.toString());
	}
	...
	public void process() throws IOException, Base64FormatException
	{
		...
		fill : while ((got = this.in.read(buffer)) > 0)
		{
			int skiped = 0;
			while (skiped < got)
			{
				...
				if (chunk[2] == 65)
				{
					this.out.write(get1(chunk, 0));
					return;
				}
				else if (chunk[3] == 65)
				{
					this.out.write(get1(chunk, 0));
					this.out.write(get2(chunk, 0));
					return;
				}
				else
				{
	...
	public String processString() throws Base64FormatException
	{
		...
		try
		{
			s = ((ByteArrayOutputStream) this.out).toString("ISO-8859-1");
		}
		catch (UnsupportedEncodingException ex)
		{
			throw new RuntimeException(
