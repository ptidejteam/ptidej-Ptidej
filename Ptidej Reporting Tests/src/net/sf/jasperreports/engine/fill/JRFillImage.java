...
public class JRFillImage extends JRFillGraphicElement implements JRImage
{
			...
			byte hAlign) throws JRException
	{
		...
		{
			this.imageHeight = Integer.valueOf(reducedHeight);
			if (getScaleImage() == JRImage.SCALE_IMAGE_REAL_SIZE)
			{
				this.imageWidth = Integer.valueOf(reducedWidth);
			}
			fitted = true;
		}
		...
		{
			this.imageHeight = Integer.valueOf(availableHeight);
			if (getScaleImage() == JRImage.SCALE_IMAGE_REAL_SIZE)
			{
				double hRatio = ((double) availableHeight) / realHeight;
				this.imageWidth = Integer.valueOf((int) (hRatio * realWidth));
			}
			fitted = true;
		}
