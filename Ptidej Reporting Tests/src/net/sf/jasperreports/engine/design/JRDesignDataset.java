 ...
 * @version $Id: JRDesignDataset.java,v 1.1 2008/09/29 16:20:08 guehene Exp $
 */
public class JRDesignDataset extends JRBaseDataset
{
	...
	private static JRDesignVariable createPageCountVariable()
	{
		...
		expression.setText("Integer.valueOf(1)");
	...
	public void setQuery(JRDesignQuery query)
	{
		...
		if (this.query != null)
		{
			((JRDesignQuery) this.query).removePropertyChangeListener(JRDesignQuery.PROPERTY_LANGUAGE, this.queryLanguageChangeListener);
		...
		if (query != null)
		{
			query.addPropertyChangeListener(JRDesignQuery.PROPERTY_LANGUAGE, this.queryLanguageChangeListener);
	...
	public void setScriptletClass(String scriptletClass)
	{
		...
		{
			((JRDesignParameter) this.parametersMap.get(JRParameter.REPORT_SCRIPTLET)).setValueClassName(scriptletClass);
		}
	...
	public void addField(JRField field) throws JRException
	{
		...
		getEventSupport().fireCollectionElementAddedEvent(PROPERTY_FIELDS, field, this.fieldsList.size() - 1);
	}
	...
	public void addSortField(JRSortField sortField) throws JRException
	{
		...
		getEventSupport().fireCollectionElementAddedEvent(PROPERTY_SORT_FIELDS, sortField, this.sortFieldsList.size() - 1);
	}
	...
	public void addGroup(JRDesignGroup group) throws JRException
	{
		...
		getEventSupport().fireCollectionElementAddedEvent(PROPERTY_GROUPS, group, this.groupsList.size() - 1);
	}
	...
	public Object clone() 
	{
		...
		if (this.parametersList != null)
		{
			...
			for(int i = 0; i < this.parametersList.size(); i++)
			{
				JRParameter parameter = 
					(JRParameter)((JRParameter)this.parametersList.get(i)).clone();
		...
		if (this.fieldsList != null)
		{
			...
			{
				JRField field = 
					(JRField)((JRField)this.fieldsList.get(i)).clone();
		...
		if (this.sortFieldsList != null)
		{
			...
			for(int i = 0; i < this.sortFieldsList.size(); i++)
			{
				JRSortField sortField = 
					(JRSortField)((JRSortField)this.sortFieldsList.get(i)).clone();
		...
		if (this.variablesList != null)
		{
			...
			{
				JRVariable variable = 
					(JRVariable)((JRVariable)this.variablesList.get(i)).clone();
		...
		if (this.groupsList != null)
		{
			...
			{
				JRGroup group = 
					(JRGroup)((JRGroup)this.groupsList.get(i)).clone();
