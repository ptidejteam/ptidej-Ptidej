...
public class MetricsGenerator {
	...
	private static final String[] METRICS_TO_COMPUTE = new String[] { "NAD",
			"NMD" };
	...
	private static void computeMetrics(final IIdiomLevelModel idiomLevelModel,
		...
		try {
			...
			final Writer writer = ProxyDisk.getInstance()
					.fileAbsoluteOutput(anOutputFileName + ".metrics");
			...
			// Header.
			{
				...
				for (int i = 0; i < metrics.length; i++) {
					...
					//		metricName)) {
					if (ArrayUtils.contains(MetricsGenerator.METRICS_TO_COMPUTE,
							metricName)) {
			...
			// Metric Values.
			final Iterator entityIterator = idiomLevelModel
				...
				if (!(firstClassEntity instanceof IGhost)) {
					...
					for (int i = 0; i < metrics.length; i++) {
						...
						if (ArrayUtils.contains(
							...
							try {
								final double value = unaryMetric.compute(
										idiomLevelModel, firstClassEntity);
								//	System.out.print("");
	...
	private static void computeMetricsFromDirectory(
			final String aSourceFilePath, final String anOutputFileName) {
	...
	private static void computeMetricsFromEclipse(final String aPluginPath,
			final String anOutputFileName) {

		final IIdiomLevelModel idiomLevelModel = ModelGenerator
				.generateModelFromJAR(aPluginPath);
		if (idiomLevelModel == null
				|| idiomLevelModel.getNumberOfConstituents() == 0) {
	...
	public static void main(final String[] args) {
		if (args.length == 1) {
			System.err.println(
					"Usage: MetricsGenerator <path to bin directory (exclusing \"bin/\"> <name of the program>");
			MetricsGenerator.computeMetricsFromDirectory(
					"../../P-MARt Workspace/" + args[0] + "/bin/", args[0]);
		}
		else {
