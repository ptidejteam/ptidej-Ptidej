...
public final class DottedLine extends Primitive implements
	...
	public DottedLine(
		...
		this(primitiveFactory, origin, new Dimension(
	...
	public void paint(final int xOffset, final int yOffset) {
		...
		int stepX = (int) (FACTOR * Drawable.DOT_LENGTH * Math.cos(alpha));
		int stepY = (int) (FACTOR * Drawable.DOT_LENGTH * Math.sin(alpha));
		int factorX = stepX > 0 ? 1 : -1;
		int factorY = stepY > 0 ? 1 : -1;
		
		this.getGraphics().setColor(FOREGROUND_COLOR);
		int movingX = x1;
		int movingY = y1;
		int nextX;
		int nextY;
