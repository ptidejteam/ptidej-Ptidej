...
public final class Composition3Test extends Primitive {
	private static Occurrence[] BuiltSolutions;

	public Composition3Test(final String name) {
		super(name);
	}

	protected void setUp() {
		if (Composition3Test.BuiltSolutions == null) {
			Composition3Test.BuiltSolutions = Primitive.automaticSolve(
	...
	public void testNumberOfSolutions() {
		Assert.assertEquals("Number of solutions", 1,
				Composition3Test.BuiltSolutions.length);
	}

	public void testSolution() {
		Assert.assertEquals("AggregateClass1 == AggregateClass1",
				"AggregateClass1",
				Composition3Test.BuiltSolutions[0]
	...
	public void testSolutionPercentage() {
		for (int i = 0; i < Composition3Test.BuiltSolutions.length; i++) {
			Assert.assertEquals("Solution with all constraints", 100,
					Composition3Test.BuiltSolutions[i].getConfidence());
		}
	}
}
