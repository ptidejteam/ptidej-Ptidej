...
public class TableModel extends AbstractTableModel implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 154852951352168656L;
	...
	private static List newVector(int size) {
	 ...
	 * @see #setValueAt
	 */
	public TableModel(List columnNames, int rowCount) {
	 ...
	 * @see #setDataVector
	 */
	public TableModel(List data, List columnNames) {
	...
	private static List nonNullVector(List v) {
	 ...
	 * @see #getDataVector
	 */
	public void setDataVector(List dataVector, List columnIdentifiers) {
	...
	private void justifyRows(int from, int to) {
		...
		for (int i = from; i < to; i++) {
			if (this.dataVector.get(i) == null) {
				this.dataVector.set(i, new ArrayList(getColumnCount()));
			}
			//	((List) dataVector.get(i)).setSize(getColumnCount());
		}
	}
	...
	public void addColumn(Object columnName, List columnData) {
		...
		if (columnData != null) {
			...
			for (int i = 0; i < columnSize; i++) {
				List row = (List) this.dataVector.get(i);
	...
	public Object getValueAt(int row, int column) {
		List rowVector = (List) this.dataVector.get(row);
	 ...
	 */
	protected static List convertToVector(Object[] anArray) {
		if (anArray == null) {
			return null;
		}
	...
	protected static List convertToVector(Object[][] anArray) {
		...
		List v = new ArrayList(anArray.length);
		for (int i = 0; i < anArray.length; i++) {
			v.add(convertToVector(anArray[i]));
		}
		return v;
	}
