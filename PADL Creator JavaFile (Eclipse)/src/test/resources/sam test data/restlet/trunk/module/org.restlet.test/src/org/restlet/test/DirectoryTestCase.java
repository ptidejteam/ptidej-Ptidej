...
public class DirectoryTestCase extends TestCase
{
	...
	public void testDirectory() throws IOException
	{
		try
		{
			...
			MyApplication application = new MyApplication(clientContainer, testDir);
			...
			String indexName) throws IOException
	{
		...
		Response response = handle(application, webSiteURL, webSiteURL, Method.GET, null,
				"1a");
		...
		response = handle(application, webSiteURL, webSiteURL + "123456.txt", Method.GET,
				null, "2b");
		assertTrue(response.getStatus().equals(Status.CLIENT_ERROR_NOT_FOUND));
		...
		{
			response = handle(application, webSiteURL, testDirectoryUrl, Method.DELETE,
					null, "6c-1");
			assertTrue(response.getStatus().equals(Status.REDIRECTION_SEE_OTHER));
			System.out.println(response.getRedirectRef());
			response = handle(application, response.getRedirectRef().getPath(), response
					.getRedirectRef().getPath(), Method.DELETE, null, "6c-2");
			assertTrue(response.getStatus().equals(Status.CLIENT_ERROR_FORBIDDEN));
			response = handle(application, webSiteURL, webSiteURL, Method.DELETE, null,
					"6c-3");
			assertTrue(response.getStatus().equals(Status.CLIENT_ERROR_FORBIDDEN));
		}
		else
		{
			//As there is no index file in the directory, the response must return the status Status.CLIENT_ERROR_NOT_FOUND  
		...
		response = handle(application, webSiteURL, baseFileUrlFrBis, Method.PUT,
				new StringRepresentation("message de test"), "7b-1");
		assertTrue(response.getStatus().equals(Status.SUCCESS_OK));
		...
		assertTrue(response.getStatus().equals(Status.SUCCESS_OK));
		response = handle(application, webSiteURL, baseFileUrlFrBis, Method.HEAD, null,
				"7b-3");
		assertTrue(response.getStatus().equals(Status.SUCCESS_OK));
		...
		response = handle(application, webSiteURL, baseFileUrlFr, Method.DELETE, null,
				"7c-1");
		assertTrue(response.getStatus().equals(Status.SUCCESS_NO_CONTENT));
		...
		response = handle(application, webSiteURL, baseFileUrlFrBis, Method.HEAD, null,
				"7c-3");
		assertTrue(response.getStatus().equals(Status.CLIENT_ERROR_NOT_FOUND));
		response = handle(application, webSiteURL, baseFileUrlFrBis, Method.DELETE, null,
				"7c-4");
		assertTrue(response.getStatus().equals(Status.CLIENT_ERROR_NOT_FOUND));
		...
		assertTrue(response.getStatus().equals(Status.SUCCESS_CREATED));
		response = handle(application, webSiteURL, baseFileUrlFrBis, Method.PUT,
				new StringRepresentation("message de test Bis"), "7d-2");
		assertTrue(response.getStatus().equals(Status.SUCCESS_OK));
	 ...
	 * @return
	 */
	private Response handle(Application application, String baseRef, String resourceRef,
			Method method, Representation entity, String testCode)
	{
	...
	private class MyApplication extends Application
	{
		...
		public MyApplication(Container container, File testDirectory)
		{
			...
			this.directory = new Directory(getContext(), getTestDirectory().toURI()
					.toString());
