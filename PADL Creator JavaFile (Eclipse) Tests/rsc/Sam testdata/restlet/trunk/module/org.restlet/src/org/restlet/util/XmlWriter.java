 ...
 * 
 * <p>This class can be used by itself or as part of a SAX event stream: it
 * takes as input a series of SAX2 ContentHandler events and uses the
 * information in those events to write an XML document. Since this class is a
 * filter, it can also pass the events on down a filter chain for further
 * processing (you can use the XMLWriter to take a snapshot of the current state
 * at any point in a filter chain), and it can be used directly as a
 * ContentHandler for a SAX2 XMLReader.</p>
 * 
 * <p>The client creates a document by invoking the methods for standard SAX2
 ...
 * </pre>
 * 
 * <p>Except that it is more efficient because it does not allocate a new empty
 ...
 * </pre>
 * 
 * <p>In fact, there is an even simpler convenience method, <var>dataElement</var>,
 * designed for writing elements that contain only character data, so the code
 * to generate the document could be shortened to</p>
 * 
 * <pre>
 * XMLWriter w = new XMLWriter();
 *
 ...
 * <h2>Whitespace</h2>
 * 
 * <p>According to the XML Recommendation, <em>all</em> whitespace in an XML
 * document is potentially significant to an application, so this class never
 * adds newlines or indentation. If you insert three elements in a row, as in</p>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * <p>You need to invoke one of the <var>characters</var> methods explicitly
 * to add newlines or indentation. Alternatively, you can use the data format 
 * mode (set the "dataFormat" property) which is optimized for writing purely 
 * data-oriented (or field-oriented) XML, and does automatic linebreaks and 
 * indentation (but does not support mixed content properly). See details below.</p>
 * 
 * <h2>Namespace Support</h2>
 * 
 * <p>The writer contains extensive support for XML Namespaces, so that a
 * client application does not have to keep track of prefixes and supply
 ...
 * </pre>
 * 
 * <p>In many cases, document authors will prefer to choose their own prefixes
 ...
 * method.</li> </ol>
 * 
 * <p>Whenever the XML writer finds a new Namespace URI, it checks to see if a
 ...
 * Namespace URI).</p>
 * 
 * <p>Before writing a document, the client can also pre-map a prefix to a
 ...
 * </pre>
 * 
 * <p>By default, the XML writer will not declare a Namespace until it is
 * actually used. Sometimes, this approach will create a large number of
 * Namespace declarations, as in the following example:</p>
 * 
 * <pre>
 * &lt;xml version="1.0" standalone="yes"?>
 *
 ...
 * </pre>
 * 
 * <p>The "rdf" prefix is declared only once, because the RDF Namespace is used
 * by the root element and can be inherited by all of its descendants; the "dc"
 ...
 * predeclare Namespaces on the root element even if they are not used there:</p>
 * 
 * <pre>
 * w.forceNSDecl("http://www.purl.org/dc/");
 * </pre>
 * 
 * <p>Now, the "dc" prefix will be declared on the root element even though
 * it's not needed there, and can be inherited by its descendants:</p>
 * 
 * <pre>
 * &lt;xml version="1.0" standalone="yes"?>
 *
 ...
 * </pre>
 * 
 * <p>This approach is also useful for declaring Namespace prefixes that be
 * used by qualified names appearing in attribute values or character data.</p>
 * 
 * <h2>Data Format</h2>
 * 
 * <p>This mode, enabled by the "dataFormat" property, pretty-prints field-oriented 
 * XML without mixed content. All added indentation and newlines will be passed on down
 ...
 *
 * <p>This writer mode will automatically place each start tag on a new line,
...
public class XmlWriter extends XMLFilterImpl
{
	...
	/**
	 * Create a new XML writer. <p>Use the specified XML reader as the parent,
	 * and write to the specified writer.</p>
	 * @param xmlreader The parent in the filter chain, or null for no parent.
	 * @param writer The output destination, or null to use standard output.
	 */
	...
	/**
	 * Write a string of character data, with XML escaping. <p>This is a
	 * convenience method that takes an XML String, converts it to a character
	 ...
	 * Namespace URI.
	 * 
	 * <p>This is a convenience method to write a complete element with
	 * character data content, including the start tag and end tag. The method
	 * provides an empty string for the Namespace URI, and empty string for the
	 * qualified name, and an empty attribute list.</p>
	 * 
	 ...
	 * 
	 * <p>This is a convenience method to write a complete element with
	 * character data content, including the start tag and end tag. This method
	 * provides an empty string for the qname and an empty attribute list.</p>
	 * 
	 * <p>This method invokes
	 * {@link #startElement(String, String, String, Attributes)}, followed by
	 * {@link #characters(String)}, followed by
	 ...
	 * 
	 * <p>This is a convenience method to write a complete element with
	 * character data content, including the start tag and end tag.</p>
	 * 
	 * <p>This method invokes
	 * {@link #startElement(String, String, String, Attributes)}, followed by
	 * {@link #characters(String)}, followed by
	 ...
	 * 
	 * <p>This method will supply an empty string for the qname, and empty
	 * string for the Namespace URI, and an empty attribute list. It invokes
	 ...
	 * 
	 * <p>This method will supply an empty string for the qname and an empty
	 ...
	 * @param uri The element's Namespace URI, or the empty string if the element
	 * has no Namespace or if Namespace processing is not being performed.
	 * @param localName The element's local name (without prefix). This parameter
	 * must be provided.
	 ...
	 * 
	 * <p>This method will supply an empty string for the qName and an empty
	 * string for the Namespace URI. It invokes
	 ...
	 * 
	 * <p>This method will supply an empty string for the qName. It invokes
	 * {@link #endElement(String, String, String)} directly.</p>
	 * 
	 * @param uri The element's Namespace URI.
	 * @param localName The element's local name.
	...
	/**
	 * Flush the output. <p>This method flushes the output stream. It is
	 * especially useful when you need to make certain that the entire document
	 * has been written to output but do not want to close the output stream.</p>
	...
	/**
	 * Force a Namespace to be declared on the root element. <p>By default, the
	 * XMLWriter will declare only the Namespaces needed for an element; as a
	 * result, a Namespace may be declared many places in a document if it is not
	 * used on the root element.</p> <p>This method forces a Namespace to be
	 * declared on the root element even if it is not used there, and reduces the
	 * number of xmlns attributes in the document.</p>
	...
	/**
	 * Force a Namespace declaration with a preferred prefix. <p>This is a
	 * convenience method that invokes {@link #setPrefix setPrefix} then
	 * {@link #forceNSDecl(java.lang.String) forceNSDecl}.</p>
	...
	/**
	 * Return the current indent step. <p>Return the current indent step: each
	 * start tag will be indented by this number of spaces times the number of
	 ...
	 * Reset the writer.
	 * 
	 * <p>This method is especially useful if the writer throws an exception
	 * before it is finished, and you want to reuse the writer for a new
	 * document. It is usually a good idea to invoke {@link #flush flush} before
	 * resetting the writer, to make sure that no output is lost.</p>
	 * 
	 * <p>This method is invoked automatically by the
	 * {@link #startDocument startDocument} method before writing a new document.</p>
	 * 
	 * <p><strong>Note:</strong> this method will <em>not</em> clear the
	 * prefix or URI information in the writer or the selected output writer.</p>
	 * 
	 * @see #flush
	 */
	public void reset()
	...
	/**
	 * Specify a preferred prefix for a Namespace URI. <p>Note that this method
	 * does not actually force the Namespace to be declared; to do that, use the
	 * {@link  #forceNSDecl(java.lang.String) forceNSDecl} method as well.</p>
	 * @param uri The Namespace URI.
	 ...
	 * 
	 * <p>This method will provide an empty string for the Namespace URI, and
	 * empty string for the qualified name, and a default empty attribute list.
	 * It invokes #startElement(String, String, String, Attributes)} directly.</p>
	 * 
	 * @param localName The element's local name.
	 ...
	 * 
	 * <p>This method will provide a default empty attribute list and an empty
	 * string for the qualified name. It invokes {@link  #startElement(String,
	 * String, String, Attributes)} directly.</p>
	 * 
	 * @param uri The element's Namespace URI.
	 * @param localName The element's local name.
	 * @exception org.xml.sax.SAXException If there is an error writing the start
	...
	/**
	 * Write out an attribute list, escaping values. The names will have prefixes
	 * added to them.
	 * @param atts The attribute list to write.
	 * @exception org.xml.SAXException If there is an error writing the attribute
	 * list, this method will throw an IOException wrapped in a SAXException.
	 */
