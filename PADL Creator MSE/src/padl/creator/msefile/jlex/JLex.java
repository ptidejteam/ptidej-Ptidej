...
/***************************************************************
  Package Declaration
  **************************************************************/
package padl.creator.msefile.jlex;
...
class CSpec {
	...
	CSpec(CLexGen lexGen) {
		...
		this.m_states.put(
			...
			Integer.valueOf(this.m_states.size()));
	...
	private void term(CNfaPair pair) throws java.io.IOException {
		...
		if (CLexGen.OPEN_PAREN == this.m_spec.m_current_token) {
			...
			if (false == (CLexGen.ANY == this.m_spec.m_current_token
					|| CLexGen.CCL_START == this.m_spec.m_current_token || (this.m_spec.m_ignorecase && isAlphaL))) {
...
class CSimplifyNfa {
	...
	private void computeClasses(CSpec m_spec) {
		...
		for (Enumeration e = m_spec.m_nfa_states.elements(); e
			...
			for (int i = 0; i < this.ccls.length; i++)
				if (clsA.get(this.ccls[i])) // a split class
					if (nfa.m_edge == i || nfa.m_edge == CNfa.CCL
							&& nfa.m_set.contains(i)) { // on A side
						Integer split = Integer.valueOf(this.ccls[i]);
	...
	private void reduce() {
		...
		for (reduced_ncols = 0;; ++reduced_ncols) {
			...
			for (j = i + 1; j < this.m_spec.m_dtrans_ncols; ++j) {
				if (-1 == this.m_spec.m_col_map[j] && true == col_equiv(i, j)) {
		...
		for (reduced_nrows = 0;; ++reduced_nrows) {
			...
			/* UNDONE: Optimize by doing all comparisons in one batch. */
			for (j = i + 1; j < nrows; ++j) {
				if (-1 == this.m_spec.m_row_map[j] && true == row_equiv(i, j)) {
...
class CLexGen {
	...
	CLexGen(String filename) throws java.io.FileNotFoundException,
		...
		this.m_tokens.put(Character.valueOf('$'), Integer.valueOf(AT_EOL));
		this.m_tokens.put(Character.valueOf('('), Integer.valueOf(OPEN_PAREN));
		this.m_tokens.put(Character.valueOf(')'), Integer.valueOf(CLOSE_PAREN));
		this.m_tokens.put(Character.valueOf('*'), Integer.valueOf(CLOSURE));
		this.m_tokens.put(Character.valueOf('+'), Integer.valueOf(PLUS_CLOSE));
		this.m_tokens.put(Character.valueOf('-'), Integer.valueOf(DASH));
		this.m_tokens.put(Character.valueOf('.'), Integer.valueOf(ANY));
		this.m_tokens.put(Character.valueOf('?'), Integer.valueOf(OPTIONAL));
		this.m_tokens.put(Character.valueOf('['), Integer.valueOf(CCL_START));
		this.m_tokens.put(Character.valueOf(']'), Integer.valueOf(CCL_END));
		this.m_tokens.put(Character.valueOf('^'), Integer.valueOf(AT_BOL));
		this.m_tokens.put(Character.valueOf('{'), Integer.valueOf(OPEN_CURLY));
		this.m_tokens.put(Character.valueOf('|'), Integer.valueOf(OR));
		this.m_tokens.put(Character.valueOf('}'), Integer.valueOf(CLOSE_CURLY));
	...
	private char[] packCode(
		...
		char prev_code[],
		int prev_read,
		int specified) throws java.io.IOException {
		if (CUtility.DEBUG) {
			CUtility.ASSERT(this.INIT_CODE == specified
					|| this.CLASS_CODE == specified
					|| this.EOF_CODE == specified
					|| this.EOF_VALUE_CODE == specified
					|| this.INIT_THROW_CODE == specified
					|| this.YYLEX_THROW_CODE == specified
					|| this.EOF_THROW_CODE == specified);
		}
		...
		while (true) {
			while (this.m_input.m_line_index >= this.m_input.m_line_read) {
				...
				if (0 == CUtility.charncmp(
					...
					if (specified == this.CLASS_CODE) {
						this.m_spec.m_class_read = prev_read;
					}
					else if (specified == this.INIT_CODE) {
						this.m_spec.m_init_read = prev_read;
					}
					else if (specified == this.EOF_CODE) {
						this.m_spec.m_eof_read = prev_read;
					}
					else if (specified == this.EOF_VALUE_CODE) {
						this.m_spec.m_eof_value_read = prev_read;
					}
					else if (specified == this.INIT_THROW_CODE) {
						this.m_spec.m_init_throw_read = prev_read;
					}
					else if (specified == this.YYLEX_THROW_CODE) {
						this.m_spec.m_yylex_throw_read = prev_read;
					}
					else if (specified == this.EOF_THROW_CODE) {
	...
	private void userDeclare() throws java.io.IOException {
		...
		while (false == this.m_input.getLine()) {
				...
				switch (this.m_input.m_line[1]) {
					...
					case 'f' :
						if (0 == CUtility.charncmp(
							...
							this.m_spec.m_dtrans_ncols = CUtility.MAX_EIGHT_BIT + 1;
							break;
						}
			...
			if (CUtility.OLD_DEBUG) {
				System.out
					.println("Line number " + this.m_input.m_line_number + ":");
	...
	private void userRules() throws java.io.IOException {
		...
		if (CUtility.DEBUG) {
			CUtility.ASSERT(END_OF_INPUT == this.m_spec.m_current_token);
		}
	...
	SparseBitSet getStates() throws java.io.IOException {
		...
		if ('<' == this.m_input.m_line[this.m_input.m_line_index]) {
			...
			while (true) {
				...
				name = new String(this.m_input.m_line, start_state, count_state);
				index = (Integer) this.m_spec.m_states.get(name);
				if (null == index) {
					/* Uninitialized state. */
					System.out.println("Uninitialized State Name: " + name);
					CError.parse_error(CError.E_STATE, this.m_input.m_line_number);
				}
		...
		while (this.m_input.m_line_index < this.m_input.m_line_read) {
			...
			if (CUtility.OLD_DEBUG) {
				System.out.println("State name \""
						...
						+ "\".");
				System.out.println("Integer index \"" + this.m_spec.m_states.size()
						+ "\".");
			}
			...
			this.m_spec.m_states.put(new String(
				this.m_input.m_line,
				start_state,
				count_state), Integer.valueOf(this.m_spec.m_states.size()));
		...
		while (true) {
			...
			while (this.m_input.m_line_index >= this.m_input.m_line_read) {
				...
				if (insinglequotes || indoublequotes) { // non-fatal
					CError.parse_error(CError.E_NEWLINE, this.m_input.m_line_number);
	...
	int advance() throws java.io.IOException {
		...
		/* End of previous regular expression?
		   Refill line buffer? */
		if (EOS == this.m_spec.m_current_token
		/* ADDED */
		|| this.m_input.m_line_index >= this.m_input.m_line_read)
		/* ADDED */
		{
		...
		while (true) {
			if (false == this.m_spec.m_in_quote
				...
				if (this.m_input.m_line_index >= this.m_input.m_line_read) {
					this.m_spec.m_current_token = EOS;
					this.m_spec.m_lexeme = '\0';
					return this.m_spec.m_current_token;
				}
			}
		...
		if (this.m_input.m_line_index > this.m_input.m_line_read) {
			System.out
				.println("m_input.m_line_index = " + this.m_input.m_line_index);
		...
		if (false == this.m_spec.m_in_quote) {
			if (false == this.m_spec.m_in_ccl
					&& CUtility.isspace(this.m_input.m_line[this.m_input.m_line_index])) {
			...
			if (saw_escape && (this.m_input.m_line_index + 1) < this.m_input.m_line_read
...
final class SparseBitSet implements Cloneable {
	...
	public boolean get(int bit) {
		...
		if (idx >= this.size || this.offs[idx] != bnum)
			return false;
		return 0 != (this.bits[idx] & (1L << (bit & BITS_M1)));
	}
	 ...
	 */
	public int size() {
		return (this.size == 0) ? 0 : ((1 + this.offs[this.size - 1]) << LG_BITS);
	}
	...
	public Enumeration elements() {
		return new Enumeration() {
			...
			public Object nextElement() {
				int r = this.bit + (SparseBitSet.this.offs[this.idx] << LG_BITS);
				advance();
	...
	public static void main(String[] args) {
		...
		for (int n = 0; n < ITER; n++) {
			...
			a.set(rr);
			v.addElement(Integer.valueOf(rr));
