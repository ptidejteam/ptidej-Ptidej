	...
	while ( getline(*file, line) )
	{
		...
		else if (num_line<=vertices)
		{
			...
			while ( std::getline(issline, word, ',') )
			{
				...
				else {double fl; from_string(word, fl); (*v).addWeight( fl, num_word-3 ); }
			}
			V.push_back(v);
		}
		...
		else if (num_line>vertices)
		{
			...
			while ( std::getline(issline, word, ',') )
			{
				...
				else {double fl; from_string(word, fl); (*e).addWeight( fl, num_word-4 ); }
			}
			/*
			if (e->label=="3")
			{
...
void Graph::initMode4(ifstream* file)
{
	...
	while ( getline(*file, line) )
	{
		...
		else if (line.substr(0,2)=="c," || line.substr(0,2)=="s,")
		{
			Vertex* v = new Vertex();
			num_word = 0;
			while ( std::getline(issline, word, ',') )
			{
				...
				else if (num_word>3) {double fl; from_string(word, fl); (*v).addWeight( fl, num_word-3 ); }
			}
			V.push_back(v);
		}
		...
		else if (line.substr(0,2)=="r,")
		{
			...
			while ( std::getline(issline, word, ',') )
			{
				...
				else if (num_word>4) {double fl; from_string(word, fl); (*e).addWeight( fl, num_word-4 ); }
			}
			/*
			if (e->label=="3")
			{
...
{
	if (notAGraph && e->from!=e->to)
		notAGraph=false;

	double fl;
	from_string(e->label, fl);

	int in;
	from_string(e->label, in);

	//cout << e->from << ">" << e->to << "\t";
 	V[e->from]->sum_out += edge_weight[in]; //in;
...
void Graph::nodeRank(double _c, double epsilon)
{
...
Graph::Graph(int mode, double jump, string path, string output, double* ww)
{
	/*
	 * PageRank with different weights for the edge labels
	 */
	notAGraph = true;
	jump_proba = jump;
	edge_weight = ww;//new double[10];
/*
	ifstream _file;
	_file.open( CONFIG );
	string _line;
	...
	{
		istringstream issline( _line );
		string word;
		int num_word;
			//first line : number of vertices, number of edges
		if (_line.substr(0,2)=="ew,")
		{
			int num_e;
			double weight_e;
			num_word = 0;
			...
			{
				num_word++;
				if (num_word==2) { int in; from_string(word, in); num_e = in; }
				else if (num_word==3) { double fl; from_string(word, fl); weight_e = fl; }
			}
			edge_weight[num_e] = weight_e;
		}
	}
*/
	...
	if (mode!=1 && mode!=2) file.open( path.c_str() );
    ...
    if (file)
    {
		...
		edges = E.size();
		norm = new double[vertices];
		nodeRank(jump, 0.000000001);
		printU(output);
		file.close();
    }
	else cout << "Not Found \n";
...
void Graph::printU(string output)
{
	ofstream file;
	string metra;
	string suffix = "";
	for (int i = 1; i <= 3; i++){
		if (i==1) suffix += "a";
		else if (i==2) suffix += "b";
		else if (i==3) suffix += "c";
		suffix += to_string(edge_weight[i]) + "_";
	}
	suffix += "j" + to_string(jump_proba) + ".csv";
	if (output=="") metra = name+"_"+suffix;
	else metra = output;//+"_"+suffix;

	file.open( metra.c_str());
	string head = "Number,Label,PageRank_score,Rank,Degree,In_Degree,Out_Degree\n";
	//head.replace(",",SEPARATOR);
	file << head;
	for (int i=0; i<vertices; i++)
	{
		file << V[i]->getNumber() << SEPARATOR << V[i]->getLabel() << SEPARATOR << u[i] << SEPARATOR << rank[i]  << SEPARATOR << V[i]->edges.size()  << SEPARATOR << V[i]->in.size()  << SEPARATOR << V[i]->out.size()  << "\n";
		/*
...
double* Graph::T(double* x, int n)
{
	...
	for (int j=0; j<n; j++)
	{
		...
		for (int k=0; k<lj; k++)
		{
			...
			double weight = V[j]->out[k]->label_weight;//label.size();
...
double* Graph::computeU(double delta, int n)
{
...
/*
		...
		//cout << "\t" << x[0] << "\t" << x[1];
		cout << "\t" << specialNorm(x,y,n) << "," << c*delta/(1-c) << "\n";
*/
	}
	while(specialNorm(x,y,n)>=c*delta/(1-c) && !notAGraph);
...
double Graph::specialNorm(double* x, double* y, int n)
{
	double norm = 0;
