...
public class InputDataGeneratorWith9Relations implements IGenerator {
	private static final class ConstituentCouple {
		...
		public ConstituentCouple(final IConstituent anOriginConstituent,
		...
		public boolean equals(final Object anotherObject) {
			...
			final ConstituentCouple anotherCouple = (ConstituentCouple) anotherObject;
	...
	private final TObjectIntMap mapOfConstituentsAndIndexes = new TObjectIntHashMap();
	...
	public InputDataGeneratorWith9Relations(final boolean withGhosts,
	...
	private void addFieldSignature(final IField aField,
			final Map aMapOfFieldsSignatures) {

		if (!this.currentConstituents.empty()) {
			final StringBuffer buffer = new StringBuffer();
			buffer.append(Access.getFieldAsString(aField.getVisibility()));
			buffer.append('@');
	...
	private void addMethodSignature(final IOperation anOperation,
			final Map aMapOfOperationsSignatures) {

		if (!this.currentConstituents.empty()) {
			final StringBuffer buffer = new StringBuffer();
			buffer.append(
					Access.getMethodAsString(anOperation.getVisibility()));
			buffer.append('@');
			...
			if (anOperation.getName().length > 0) {
				...
				final Iterator iteratorOnParameters = anOperation
			...
			buffer.append("@[");
			final Iterator iterator = anOperation
					.getIteratorOnConstituents(IParameter.class);
	...
	private void addRelationBetweenConstituents(
		...
		if (this.isInterestingOriginEntity(anOriginConstituent)
			...
			final ConstituentCouple couple = new ConstituentCouple(
					anOriginConstituent, aTargetConstituent);
	...
	private void addRelationFromCurrentConstituent(
		...
		try {
			final IConstituent originConstituent = (IConstituent) this.currentConstituents
					.peek();
			this.addRelationBetweenConstituents(originConstituent,
					aTargetConstituent, someRelationships);
		}
	...
	public void close(final IAbstractModel p) {
		...
		while (iterator.hasNext()) {
			...
			if (target instanceof IPackage) {
				...
				final String operationSignature = (String) this.mapOfOperationsSignatures
						.get(target);
				...
				final String fieldSignature = (String) this.mapOfFieldsSignatures
						.get(target);
		...
		this.outputRelation(this.relationsReturnTypesOfMethods, 7,
				this.finalResult);
		this.outputRelation(this.relationsParameterTypesOfMethods, 8,
				this.finalResult);
		this.outputRelation(this.relationsContains, 9, this.finalResult);

		this.finalResult.insert(0,
				index + "," + this.totalNumberOfRelations + "\n");
	}
	...
	public void open(final IAbstractModel p) {
		...
		final IPackage rootPackage = Factory.getInstance()
				.createPackage(new char[] { '.' });
	...
	public void open(final IClass p) {
		...
		while (iterator.hasNext()) {
			final IFirstClassEntity targetEntity = (IFirstClassEntity) iterator
					.next();
			this.addRelationFromCurrentConstituent(targetEntity,
	...
	protected void open(final IOperation p) {
		if (this.isInterestingOriginEntity(this.peekCurrentConstituent())) {
			...
			this.constituents.add(p);
			this.addRelationFromCurrentConstituent(p, this.relationsContains);
			final Iterator iterator = p
					.getIteratorOnConstituents(IParameter.class);
			while (iterator.hasNext()) {
				final IParameter parameter = (IParameter) iterator.next();
				this.addRelationBetweenConstituents(parameter.getType(), p,
						this.relationsParameterTypesOfMethods);
			}
			if (p instanceof IMethod) {
				this.addRelationBetweenConstituents(
						this.abstractModel.getTopLevelEntityFromID(
								((IMethod) p).getReturnType()),
						p, this.relationsReturnTypesOfMethods);
			}
		}
	...
	private void outputRelation(final List someRelationships, final int aLabel,
			final StringBuffer aBuffer) {

		final Iterator relationsIterator = someRelationships.iterator();
		while (relationsIterator.hasNext()) {
			final ConstituentCouple couple = (ConstituentCouple) relationsIterator
					.next();
			final IConstituent originConstituent = couple
					.getOriginConstituent();
			final IConstituent targetConstituent = couple
					.getTargetConstituent();

			if (this.constituents.contains(targetConstituent)) {
				final int indexOfOriginConstituent = this.mapOfConstituentsAndIndexes
						.get(originConstituent.getPath());
				final int indexOfTargetConstituent = this.mapOfConstituentsAndIndexes
	...
	public void visit(final IAggregation p) {
		this.addRelationFromCurrentConstituent(p.getTargetEntity(),
	...
	public void visit(final IField p) {
		if (this.isInterestingOriginEntity(this.peekCurrentConstituent())) {
			...
			IEntity entity = this.abstractModel
					.getTopLevelEntityFromID(p.getType());
			if (entity == null) {
				entity = (IEntity) this.abstractModel
						.getConstituentFromID(p.getType());
			}
			this.addRelationBetweenConstituents(entity, p,
	...
	public void visit(final IMethodInvocation p) {
		if (p.getCalledMethod() != null) {
			if (Arrays.equals(p.getCalledMethod().getName(),
					new char[] { '=' })) {

				final IFirstClassEntity entity = p.getFieldDeclaringEntity();
				if (this.isInterestingOriginEntity(
						this.peekLastButOneConstituent())
				...
				final IConstituent originEntity = (IConstituent) this
				...
				if (this.isInterestingOriginEntity(originEntity)
					...
					// to do that!
					final IConstituent constituent = p.getTargetEntity()
							.getConstituentFromID(p.getCalledMethod().getID());
					if (constituent != null) {
						this.addRelationFromCurrentConstituent(constituent,
