...
encodingFound = true;
 ...
 else if(jct.test.rsc.snpsht.parser.revml.RevMLDocCommonsStrings.FILE_REV_DELTA_TYPE_ATTRIBUTE.equals(aName.toLowerCase().trim())) 
{
java.lang.String type = attributes.getValue(i);
if(type.compareTo(jct.test.rsc.snpsht.parser.revml.RevMLDocCommonsStrings.FILE_REV_UDIFF_DELTA_ATTR_VALUE) == 0) 
{
java.lang.System.out.println(jct.test.rsc.snpsht.parser.revml.RevMLDocCommonsStrings.FILE_REV_UDIFF_DELTA_ATTR_VALUE);

}
 else 
{
this.getFsm().changeState(new jct.test.rsc.snpsht.parser.revml.sax.fsm.ErrorStateRevMLState(this.getFsm(), this, this.getManager(), "Unexpected " + jct.test.rsc.snpsht.parser.revml.RevMLDocCommonsStrings.FILE_REV_DELTA_TYPE_ATTRIBUTE + " attribute value: " + type, jct.test.rsc.snpsht.parser.revml.sax.fsm.ErrorStateRevMLState.MISFORMED_REVML_DOCUMENT));
return;
...
{
this.getFsm().changeState(new jct.test.rsc.snpsht.parser.revml.sax.fsm.ErrorStateRevMLState(this.getFsm(), this, this.getManager(), "Delta marker have no encoding or type attribute.", jct.test.rsc.snpsht.parser.revml.sax.fsm.ErrorStateRevMLState.MISFORMED_REVML_DOCUMENT));
...
java.lang.String targetFilePath = jct.test.rsc.snpsht.serializer.verfilesystem.VerFsCommonStrings.REVS_FILES_PATH + this.getManager().toRelativePath(this.getFile().getPath()).replace("/", java.io.File.separator) + "." + this.getFile().getRevID().getValue().replace('.', '_') + ".diff";
try
{
...
{
return jct.test.rsc.snpsht.parser.revml.RevMLDocCommonsStrings.FILE_REV_DELTA_MARKER;
