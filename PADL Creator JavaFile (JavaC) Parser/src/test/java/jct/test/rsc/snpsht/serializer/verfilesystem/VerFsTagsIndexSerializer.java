...
public class VerFsTagsIndexSerializer extends AbstractVerFsSerializer {
	private IVerFsManager manager;

	public VerFsTagsIndexSerializer(IVerFsManager manager) {
    ...
    public void serialize() throws IOException {
		IFsFileEntity entity =
			this.manager.getSourceManager().add(
				VerFsCommonStrings.TAGS_INDEX_XML_FILE_NAME);
		...
		{
			writer.write("<" + VerFsCommonStrings.TAGS_MAKER + ">");
			{
				for (IVerFsTag tag : this.manager.getAllTags()) {
					writer.write("<" + VerFsCommonStrings.TAG_MAKER + " "
							+ VerFsCommonStrings.TAG_ID_ATTRIBUTE + "=\""
							+ VerFsCommonStrings.ASCII2XML(tag.getValue())
							+ "\">");
					{
						writer.write(genMarker(
							VerFsCommonStrings.TAG_ID_MAKER,
							VerFsCommonStrings.ASCII2XML(tag.getValue())));
						...
						{
							VerFsFileRev[] files =
								tag.getChildren().toArray(new VerFsFileRev[0]);
							Arrays.sort(
								files,
