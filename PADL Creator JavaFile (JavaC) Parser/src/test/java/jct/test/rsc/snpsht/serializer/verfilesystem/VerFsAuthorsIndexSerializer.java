...
public class VerFsAuthorsIndexSerializer extends AbstractVerFsSerializer {
	private IVerFsManager manager;

	public VerFsAuthorsIndexSerializer(IVerFsManager manager) {
    ...
    public void serialize() throws IOException {
		IFsFileEntity entity =
			this.manager.getSourceManager().add(
				VerFsCommonStrings.AUTHORS_INDEX_XML_FILE_NAME);
		...
		{
			writer.write("<" + VerFsCommonStrings.AUTHORS_MAKER + ">");
			{
				for (IVerFsAuthor author : this.manager.getAllAuthors()) {
					writer.write("<" + VerFsCommonStrings.AUTHOR_MAKER + " "
							+ VerFsCommonStrings.AUTHOR_NAME_ATTRIBUTE + "=\""
							+ VerFsCommonStrings.ASCII2XML(author.getValue())
							+ "\">");
					{
						writer.write(genMarker(
							VerFsCommonStrings.AUTHOR_NAME_MAKER,
							VerFsCommonStrings.ASCII2XML(author.getValue())));
