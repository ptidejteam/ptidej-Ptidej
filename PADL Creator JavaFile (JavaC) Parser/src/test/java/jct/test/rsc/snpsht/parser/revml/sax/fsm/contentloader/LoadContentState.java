...
public class LoadContentState extends AbstractStringLoaderState {
	private IContentWriter writer;

	public LoadContentState(
		...
		if (name.trim().toLowerCase().compareTo(getWaitedMarker()) == 0) {
			...
			getFsm().changeState(
				new ErrorStateRevMLState(
					...
					this,
					getManager(),
					"Content marker have no encoding attribute.",
					ErrorStateRevMLState.MISFORMED_REVML_DOCUMENT));
		} else {
	...
	protected String getWaitedMarker() {
		return RevMLDocCommonsStrings.FILE_REV_CONTENT_MARKER;
	}
