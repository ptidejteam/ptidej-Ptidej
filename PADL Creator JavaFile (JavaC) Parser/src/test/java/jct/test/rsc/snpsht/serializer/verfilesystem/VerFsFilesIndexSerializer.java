...
public class VerFsFilesIndexSerializer extends AbstractVerFsSerializer {
	private IVerFsManager manager;

	public VerFsFilesIndexSerializer(IVerFsManager manager) {
		...
		writer.write(VerFsCommonStrings.DTD_MARKER);
		{
			writer.write("<" + VerFsCommonStrings.FILES_MAKER + ">");
			{
				writer.write(genMarker(
					VerFsCommonStrings.REPO_ROOT_MAKER,
					VerFsCommonStrings.PATH2XML(this.manager
						.getRoot()
						.getPath())));

				for (IVerFsFile file : this.manager.getAllFiles()) {
							...
							+ "\">");
					{
						...
						writer.write(genMarker(
							...
							VerFsCommonStrings.PATH2XML(file
								.getFileLocation()
								.getId())));
