...
import padl.statement.kernel.impl.Statement;
...
public class MethodCloneTest extends TestCase {
	...
	public void testStartCloneSessiononMethods() {
		...
		ModelStatistics m1Listener = (ModelStatistics) m1
				.getIteratorOnModelListeners().next();
		ModelStatistics m1CloneListener = (ModelStatistics) ((Operation) m1
				.getClone()).getIteratorOnModelListeners().next();

		ModelStatistics m2Listener = (ModelStatistics) m2
				.getIteratorOnModelListeners().next();
		ModelStatistics m2CloneListener = (ModelStatistics) ((Operation) m2
				.getClone()).getIteratorOnModelListeners().next();
	...
	public void testStartCloneSessionOnMethodFields() {
		...
		Statement stmt1 = (Statement) m1
				.getConstituentFromName(statementLiteralOne);
		Statement clonedStmt1 = (Statement) ((Operation) m1.getClone())
				.getConstituentFromName(statementLiteralOne);

		Statement stmt2 = (Statement) m2
				.getConstituentFromName(statementLiteralTwo);
		Statement clonedStmt2 = (Statement) ((Operation) m2.getClone())
				.getConstituentFromName(statementLiteralTwo);
