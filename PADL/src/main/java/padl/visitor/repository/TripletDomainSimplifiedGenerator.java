 ...
 * @since  2007/02/01
 */
public class TripletDomainSimplifiedGenerator implements IGenerator {
	...
	public void open(final IClass aClass) {
		...
		final Iterator iteratorOnInheritedEntities = aClass
				.getIteratorOnInheritedEntities();
		while (iteratorOnInheritedEntities.hasNext()) {
			final IFirstClassEntity firstClassEntity = (IFirstClassEntity) iteratorOnInheritedEntities
					.next();
			this.output.append(this.getCurrentEntityName());
			this.output.append(" inherits ");
			this.output.append(Util.computeFullyQualifiedName(this.model,
					firstClassEntity));
			this.output.append('\n');
		}

		final Iterator iteratorOnImplementedEntities = aClass
				.getIteratorOnImplementedInterfaces();
		while (iteratorOnImplementedEntities.hasNext()) {
			final IFirstClassEntity firstClassEntity = (IFirstClassEntity) iteratorOnImplementedEntities
					.next();
			this.output.append(this.getCurrentEntityName());
	...
	public void open(final IInterface anInterface) {
		...
		final Iterator iteratorOnInheritedEntities = anInterface
				.getIteratorOnInheritedEntities();
		while (iteratorOnInheritedEntities.hasNext()) {
			final IFirstClassEntity firstClassEntity = (IFirstClassEntity) iteratorOnInheritedEntities
					.next();
	...
	private void visit(final IRelationship aRelationship,
		...
		if (!Arrays.equals(Util.computeFullyQualifiedName(this.model,
