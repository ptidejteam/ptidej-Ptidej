...
class MethodInvocation extends Constituent implements IMethodInvocation {
	private class NonConcurrentIterator implements Iterator, Serializable {
		...
		public boolean hasNext() {
			final boolean hasNext = this.index < MethodInvocation.this.callingFieldsSize;
			if (!hasNext) {
		...
		public Object next() {
			try {
				...
				final Object next = MethodInvocation.this.callingFields[this.index];
				this.index++;
				MethodInvocation.this.nciIterating++;
				return next;
			}
	...
	public void addCallingField(final IField aField) {
		...
		if (minCapacity > oldCapacity) {
			...
			int newCapacity = GenericContainerConstants.PRIME_NUMBERS[this.indexInPrimeNumbersArray];
			if (newCapacity < minCapacity)
				newCapacity = minCapacity;
			this.callingFields = new IField[newCapacity];
			System.arraycopy(oldData, 0, this.callingFields, 0,
	...
	public boolean equals(final IMethodInvocation anotherMethodInvocation) {
		...
		if (this.getCalledMethod() != null) {
			if (anotherMethodInvocation.getCalledMethod() == null) {
				...
				areEqual = areEqual && this.getCalledMethod()
						.equals(anotherMethodInvocation.getCalledMethod());
			}
		}
		if (this.getFieldDeclaringEntity() != null) {
			if (anotherMethodInvocation.getFieldDeclaringEntity() == null) {
				...
				areEqual = areEqual && this.getFieldDeclaringEntity().equals(
						anotherMethodInvocation.getFieldDeclaringEntity());
			}
		}
		if (this.getFirstCallingField() != null) {
			if (anotherMethodInvocation.getFirstCallingField() == null) {
				...
				areEqual = areEqual && this.getFirstCallingField()
						.equals(anotherMethodInvocation.getFirstCallingField());
			}
		}

		return areEqual
				&& this.getTargetEntity()
						.equals(anotherMethodInvocation.getTargetEntity())
	...
	public IField getFirstCallingField() {
		...
		return (this.callingFields == null || this.callingFields.length == 0)
				? null
	...
	public char[] getName() {
		...
		if (this.entityDeclaringField == null) {
			entityDeclaringFieldName = Constants.DEFAULT_METHODINVOCATION_NO_FIELD_ENTITY;
		}
		else {
	...
	public void performCloneSession() {
		final MethodInvocation clonedMethodInvocation = (MethodInvocation) this
				.getClone();
		...
		if (this.calledMethod != null) {
			...
			//	}
			clonedMethodInvocation.calledMethod = (IOperation) this.calledMethod
					.getClone();
		}
		...
		clonedMethodInvocation.callingFields = new IField[this.callingFieldsSize];
		for (int i = 0; i < this.callingFieldsSize; i++) {
			...
			((MethodInvocation) this
					.getClone()).callingFields[i] = (IField) field.getClone();
		}

		if (this.entityDeclaringField != null) {
			clonedMethodInvocation.entityDeclaringField = (IFirstClassEntity) this.entityDeclaringField
					.getClone();
		}

		if (this.targetEntity != null) {
			clonedMethodInvocation.targetEntity = (IFirstClassEntity) this.targetEntity
					.getClone();
		}
	}
	...
	public String toString() {
		...
		if (this.entityDeclaringField == null) {
			entityDeclaringFieldName = Constants.DEFAULT_METHODINVOCATION_NO_DECLARING_ENTITY;
		}
		else {
			entityDeclaringFieldName = this.entityDeclaringField.getName();
		}
		if (this.calledMethod == null) {
			invokedMethodName = Constants.DEFAULT_METHODINVOCATION_NO_INVOKED_METHOD;
		}
		else {
			invokedMethodName = this.calledMethod.getName();
		}
