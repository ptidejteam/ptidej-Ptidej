...
public class DomainGenerator2Custom extends DomainGenerator implements IMotifWalker {
	...
	private void close(final char[] name,
			final Iterator anIteratorOnSuperEntities) {

		final char[] claireIdentifier = DomainGenerator
				.convertToClaireIdentifier(name);
		...
		while (anIteratorOnSuperEntities.hasNext()) {
			this.buffer.append(DomainGenerator.convertToClaireIdentifier(
	...
	public void close(final IAbstractModel p) {
		...
		while (iterator.hasNext()) {
			final IFirstClassEntity firstClassEntity = (IFirstClassEntity) iterator
					.next();
			this.buffer.append(DomainGenerator
	...
	public void visit(final IAggregation p) {
		final char[] pEntityName = DomainGenerator
				.convertToClaireIdentifier(p.getTargetEntity().getID());
	...
	public void visit(final IField p) {
		if (this.belongsToDomain(p.getType())) {
			char[] pEntityName = DomainGenerator
					.convertToClaireIdentifier(p.getType());
