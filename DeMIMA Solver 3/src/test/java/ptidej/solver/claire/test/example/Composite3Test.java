...
public class Composite3Test extends Primitive {
	...
	public Composite3Test(final String name) {
		super(name);
	}
	...
	protected void setUp()
		...
		if (Composite3Test.FoundSolutions == null) {
			...
			final ICodeLevelModel codeLevelModel = Factory.getInstance()
					.createCodeLevelModel("ptidej.example.composite3");
			try {
				codeLevelModel
						.create(new CompleteClassFileCreator(new String[] {
								"../DeMIMA/target/test-classes/ptidej/example/composite3/" }));
			}
			...
			// Expected solutions.
			Composite3Test.ExpectedSolutions = SolutionReader
					.getExpectedSolutions("Composite3", codeLevelModel);
			Composite3Test.NumberOfExpectedSolutions = SolutionReader
					.getExpectedNumberOfSolutions("Composite3", codeLevelModel);

			// Solutions found.
			Composite3Test.FoundSolutions = this.testDesignPattern(Composite3Test.class,
	...
	public void testNumberOfSolutions() {
		Assert.assertEquals("Number of solutions",
				Composite3Test.NumberOfExpectedSolutions,
				Composite3Test.FoundSolutions.length);
	}

	public void testSolutions() {
		for (int i = 0; i < Composite3Test.NumberOfExpectedSolutions; i++) {
			Assert.assertEquals("", Composite3Test.ExpectedSolutions[i],
					Composite3Test.FoundSolutions[i]);
		}
	}
