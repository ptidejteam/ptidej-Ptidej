/***** CLAIRE Compilation of file d:\claire\v3.2\src\compile\copt.cl 
...
#include <Optimize.h>
#include <Generate.h>
void  inline_exp_c_producer1(c_producer *v7227,Call_method1 *v1140,OID v15308)
{ GC_BIND;
  { method * v7237 = v1140->arg;
    property * v7240 = v7237->selector;
    OID  v11032 = GC_OID(car_list(v1140->args));
    if (_oid_(v7237) == Generate._starch_sign_star->value)
     { princ_string("(-");
    ...
    else if ((_oid_(v7240) == Generate._starnot_star->value) && 
        (_inf_equal_type(GC_OBJECT(ClaireType,OBJECT(ClaireType,(*Optimize.c_type)(v11032))),Kernel._boolean) == CTRUE))
     { princ_string("((");
      (*Generate.expression)(v11032,
        v15308);
      princ_string(" == CTRUE) ? Kernel.false : Kernel.true)");
      } 
    else print_external_call_c_producer(v7227,v1140,v15308);
      } 
  GC_UNBIND;} 

void  inline_exp_c_producer2(c_producer *v7227,Call_method2 *v1140,OID v15308)
{ GC_BIND;
  { method * v7237 = v1140->arg;
    property * v7240 = v7237->selector;
...
void  inline_exp_c_producer3(c_producer *v7227,Call_method *v1140,OID v15308)
{ GC_BIND;
  { method * v7237 = v1140->arg;
    ...
    if ((_oid_(v7237) == Generate._starnth_equal_list_star->value) && 
     ...
     { princ_string("CL_ADD_SLOT(");
      (*Generate.expression)(v11032,
        v15308);
      princ_string(",");
      ident_symbol(OBJECT(symbol,(*Kernel.name)(GC_OID(getC_any(v11032)))));
      princ_string(",");
      (*Generate.expression)(v11033,
        v15308);
      princ_string(",");
...
void  print_external_call_c_producer(c_producer *v7227,Call_method *v1140,OID v15308)
{ GC_BIND;
  { method * v7237 = v1140->arg;
...
void  inline_exp_c_producer5(c_producer *v7227,Call *v1140,OID v15308)
{ GC_BIND;
                ...
                { v_and = ((v11438->length > 0) ? CTRUE : CFALSE);
                  if (v_and == CFALSE) v1934 =CFALSE; 
                      ...
                      { v_or = ((v6058 == 0) ? CTRUE : CFALSE);
                        if (v_or == CTRUE) v_and =CTRUE; 
                            ...
                            { OID gc_local;
                              ...
                              { GC_LOOP;
                                if (BCONTAIN(status_I_restriction(OBJECT(restriction,v7237)),1))
                                 { v4290 = Kernel.ctrue;
                                  break;} 
                                GC_UNLOOP;} 
                              } 
                    ...
                    if (v_and == CFALSE) v1934 =CFALSE; 
                          ...
                          (v7236->length <= 4)) ? CTRUE : ((v11438->length < Generate.FCLimit->value) ? CTRUE : CFALSE));
                      if (v_and == CFALSE) v1934 =CFALSE; 
                      else v1934 = CTRUE;} 
                    } 
                  } 
            ...
            { list * v3814;
              { { bag *v_list; OID v_val;
                  ...
                  { v7248 = (*(v_list))[CLcount];
                    { ClaireClass * v7227 = class_I_type(OBJECT(ClaireType,v7248));
                      ClaireClass * v11640 = sort_I_class(v7227);
                      if (v11640 == Kernel._object)
                       v_val = _oid_(v7227);
                      else v_val = _oid_(v11640);
                        } 
              ...
              if (v7237 != last_list(v10564))
               { princ_string("(INHERIT(");
                ...
                princ_string("((");
                ident_class(domain_I_restriction(OBJECT(restriction,v7237)));
                princ_string(" *) ");
...
OID  c_sorted_arg_any(OID v7248,ClaireClass *v7243,OID v15308,ClaireBoolean *v4284)
{ GC_BIND;
  if (v4284 == CTRUE)
   princ_string("((int) ");
  if ((INHERIT(OWNER(v7248),Optimize._to_CL)) && (sort_I_class(OBJECT(to_CL,v7248)->set_arg) == v7243))
...
OID  c_sorted_args_Call(Call *v7248,list *v11439,OID v15308,ClaireBoolean *v4284)
{ GC_BIND;
  { OID Result = 0;
    { int  v7233 = 0;
      { OID gc_local;
...
void  bitvector_I_c_producer(c_producer *v7227,OID v7248)
{ GC_BIND;
  if (INHERIT(OWNER(v7248),Kernel._integer))
   bitvectorSum_integer(v7248);
  else if (INHERIT(OWNER(v7248),Optimize._to_CL))
   bitvector_I_c_producer(v7227,GC_OID(OBJECT(to_CL,v7248)->arg));
  else if (INHERIT(OWNER(v7248),Optimize._to_protect))
   bitvector_I_c_producer(v7227,GC_OID(OBJECT(to_protect,v7248)->arg));
...
void  signature_I_c_producer(c_producer *v7227,OID v7248)
{ GC_BIND;
  if (INHERIT(OWNER(v7248),Kernel._list))
   ...
   signature_I_c_producer(v7227,GC_OID(OBJECT(to_C,v7248)->arg));
  else if (INHERIT(OWNER(v7248),Optimize._to_protect))
   signature_I_c_producer(v7227,GC_OID(OBJECT(to_protect,v7248)->arg));
...
{ GC_BIND;
  { OID Result = 0;
    if (INHERIT(OWNER(v7248),Optimize._to_CL))
     Result = getC_any(GC_OID(OBJECT(to_CL,v7248)->arg));
    else if (INHERIT(OWNER(v7248),Optimize._to_protect))
     Result = getC_any(GC_OID(OBJECT(to_protect,v7248)->arg));
...
void  gassign_c_producer(c_producer *v7227,Gassign *v1140,OID v15308)
...
void  call_slot_c_producer(c_producer *v7227,Call_slot *v1140,OID v15308)
{ GC_BIND;
  if (v1140->test == CTRUE)
   { princ_string((((INHERIT(v1140->selector->srange,Kernel._import)) || 
...
void  call_table_c_producer(c_producer *v7227,Call_table *v1140,OID v15308)
{ GC_BIND;
...
void  call_array_c_producer(c_producer *v7227,Call_array *v1140,OID v15308)
...
OID  update_c_producer(c_producer *v7227,Update *v1140,OID v15308)
{ GC_BIND;
  { OID Result = 0;
...
void  object_test_c_producer(c_producer *v7227,OID v11032,ClaireBoolean *v3475,OID v15308)
{ princ_string("(CTAG(");
  (*Generate.expression)(v11032,
    v15308);
  princ_string(") ");
...
void  exp_to_protect_c_producer(c_producer *v7227,to_protect *v1140,OID v15308)
{ if ((Optimize.OPT->protection == CTRUE) && 
      ((need_protect_any(v1140->arg) == CTRUE) && 
        ((Optimize.OPT->alloc_stack != CTRUE) || 
            (_inf_equal_type(GC_OBJECT(ClaireType,OBJECT(ClaireType,(*Optimize.c_type)(v1140->arg))),Kernel._tuple) != CTRUE)))) 
  { { OID  v7248 = v1140->arg;
      ...
      if (INHERIT(v7243,Kernel._object))
       { ident_class(psort_any(GC_OID((*Core.glb)(GC_OID((*Optimize.c_type)(v7248)),
          _oid_(v7243)))));
        princ_string(",");
        (*Generate.expression)(v1140->arg,
          v15308);
...
void  macro_c_producer(c_producer *v7227)
{ ;} 

void  init_var_c_producer(c_producer *v7227,ClaireClass *v7243)
{ ;} 

void  any_interface_c_producer(c_producer *v7227)
{ princ_string("OID");
  } 

void  pointer_cast_c_producer(c_producer *v7227,ClaireClass *v7243)
{ princ_string("(");
  ident_class(v7243);
  princ_string(" *)");
  } 

void  exp_Assign_c_producer(c_producer *v7227,Assign *v1140,OID v15308)
{ GC_BIND;
  { OID  v7248 = v1140->arg;
    OID  v7247 = v1140->var;
...
void  stat_handle_c_producer(c_producer *v7227,ClaireHandle *v1140,OID v7243,OID v15308)
{ GC_BIND;
  new_block_void();
...
void  stat_construct_c_producer(c_producer *v7227,Construct *v1140,OID v7243,OID v15308)
{ GC_BIND;
...
void  stat_while_c_producer(c_producer *v7227,While *v1140,OID v7243,OID v15308)
{ GC_BIND;
  new_block_void();
  { ClaireBoolean * v11201 = ((c_func_any(GC_OID(v1140->test)) == CTRUE) ? ((v1140->other != CTRUE) ? ((gc_usage_any(GC_OID(v1140->test),OBJECT(ClaireBoolean,v15308)) == Kernel.cfalse) ? CTRUE: CFALSE): CFALSE): CFALSE);
    ...
    { ClaireBoolean * v13790 = ((Optimize.OPT->loop_gc == CTRUE) ? ((gc_usage_any(GC_OID(v1140->arg),CTRUE) != Kernel.cfalse) ? CTRUE: CFALSE): CFALSE);
      if (v13790 == CTRUE)
       ...
       { princ_string("// HOHO, GC_LOOP not needed !");
        breakline_void();
        } 
      { OID  v2273;
        if (Kernel._string == OWNER(v7243))
...
void  stat_gassign_c_producer(c_producer *v7227,Gassign *v1140,OID v7243,OID v15308)
{ GC_BIND;
  new_block_void();
  princ_string("OID truc;");
...
void  stat_for_c_producer(c_producer *v7227,For *v1140,OID v7243,OID v15308)
{ GC_BIND;
  { char * v7247 = GC_STRING(string_I_c_producer1(v7227,GC_OBJECT(Variable,v1140->var)));
    new_block_void();
    if (Optimize.OPT->loop_gc == CTRUE)
     { princ_string("OID gc_local;");
      breakline_void();
...
void  stat_iteration_c_producer(c_producer *v7227,Iteration *v1140,OID v7243,OID v15308)
{ GC_BIND;
  if (boolean_I_any(v7243) != CTRUE)
...
void  stat_super_c_producer(c_producer *v7227,Super *v1140,OID v7243,OID v15308)
{ GC_BIND;
...
void  stat_let_c_producer(c_producer *v7227,Let *v1140,OID v7243,OID v15308)
{ GC_RESERVE(8);  // v3.0.55 optim !
  { char * v11501 = string_v((*Kernel.string_I)(Generate.PRODUCER->value,
      ...
      { GC_LOOP;
        ...
        if (INHERIT(OWNER(v1140->arg),Language._Let))
         { GC__ANY(v1140 = OBJECT(Let,v1140->arg), 2);
          GC__OID(v7247 = (*Kernel.string_I)(Generate.PRODUCER->value,
