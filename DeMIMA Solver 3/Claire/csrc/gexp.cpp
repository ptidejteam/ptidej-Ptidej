/***** CLAIRE Compilation of file d:\claire\v3.2\src\compile\gexp.cl 
...
{ GC_RESERVE(1);  // HOHO v3.0.55 optim !
  { ClaireBoolean *Result ;
    if (INHERIT(OWNER(v1140),Kernel._bag))
       ...
       { ClaireBoolean *v_and;
        { v_and = ((OBJECT(Construct,v1140)->args->length < 15) ? CTRUE : CFALSE);
          if (v_and == CFALSE) Result =CFALSE; 
              ...
              { ClaireBoolean * v8222;{ OID  v868;
                  { set * v8224;{ set * v4832 = set::empty(Kernel.emptySet);
                      { OID gc_local;
                        ITERATE(v7248);
                        bag *v7248_support;
                      ...
                      v8224 = GC_OBJECT(set,v4832);
                      } 
                    
                    v868=_oid_(v8224);} 
                  v8222 = boolean_I_any(v868);
                  } 
                
                v15331=_oid_(v8222);} 
              v_and = ((v15331 != Kernel.ctrue) ? CTRUE : CFALSE);
              } 
            if (v_and == CFALSE) Result =CFALSE; 
            else Result = CTRUE;} 
          } 
                  ...
                  { int  v7239 = 4;
                    ...
                    { OID gc_local;
                      ...
                      { GC_LOOP;
                        ...
                        else { v2790 = Kernel.ctrue;
                            break;} 
                          GC_UNLOOP;} 
                      } 
                    } 
                  v_and = boolean_I_any(v2790);
                  } 
                if (v_and == CFALSE) Result =CFALSE; 
                else Result = CTRUE;} 
              } 
     ...
     Result = c_func_any(GC_OID(OBJECT(to_protect,v1140)->arg));
     ...
     Result = c_func_any(GC_OID(_oid_(OBJECT(Call_method,v1140)->args)));
     ...
     Result = c_func_any(GC_OID(OBJECT(Call_array,v1140)->arg));
      ...
      ((INHERIT(OWNER(v1140),Language._Cast)) ?
        ...
        ((INHERIT(OWNER(v1140),Optimize._to_C)) ?
          c_func_any(GC_OID(OBJECT(to_C,v1140)->arg)) :
          ((INHERIT(OWNER(v1140),Optimize._to_CL)) ?
            c_func_any(GC_OID(OBJECT(to_CL,v1140)->arg)) :
    ...
    if ((INHERIT(OWNER(v7248),Language._Call_method2)) && ((_oid_(OBJECT(Call_method,v7248)->arg) == Generate._star_plus_integer_star->value) && 
     ...
     { OID  v11392;
      { if (INHERIT(OWNER(v7248),Optimize._to_protect))
         v11392 = OBJECT(to_protect,v7248)->arg;
        else v11392 = v7248;
          GC_OID(v11392);} 
...
void  expression_to_protect(to_protect *v1140,OID v15308)
...
void  expression_to_CL(to_CL *v1140,OID v15308)
{ GC_BIND;
...
void  expression_to_C(to_C *v1140,OID v15308)
{ GC_BIND;
...
void  expression_C_cast(C_cast *v1140,OID v15308)
...
void  bool_exp_to_CL_Generate(to_CL *v1140,ClaireBoolean *v3475,OID v15308)
{ GC_BIND;
...
{ GC_BIND;
  { method * v7237 = v1140->arg;
    ...
    if ((_oid_(v7237) == Generate._starnot_star->value) && 
        (INHERIT(OWNER(v11032),Optimize._to_CL)))
     (*Optimize.bool_exp)(GC_OID((*Kernel.arg)(v11032)),
...
{ GC_BIND;
  { method * v7237 = v1140->arg;
    ...
    list * v10533 = GC_OBJECT(list,OBJECT(producer,Generate.PRODUCER->value)->open_comparators);
    OID  v11032 = (*(v1140->args))[1];
...
{ GC_BIND;
  { ClaireBoolean *Result ;
    if (INHERIT(OWNER(v7248),Optimize._to_CL))
     Result = bool_exp_ask_any(OBJECT(to_CL,v7248)->arg);
