/***** CLAIRE Compilation of file d:\claire\v3.2\src\meta\file.cl 
...
{ next_meta_reader(Reader.reader);
  return (_void_(close_exception(((general_error *) (*Core._general_error)(_string_("[117] loose delimiter ~S in program [line ~A ?]"),
...
{ { ClaireBoolean *Result ;
    { ClaireBoolean *v_or;
      { { OID  g0074UU;
          { g0074UU= _oid_(CFALSE);
            while ((useless_c_integer(firstc_meta_reader(self)) == CTRUE))
            { if (firstc_meta_reader(self) == 10)
               { g0074UU = Kernel.ctrue;
                break;} 
              next_meta_reader(self);
              } 
            } 
          v_or = boolean_I_any(g0074UU);
          } 
  ...
  { { Variable *Result ;
    ...
    { Variable *Result ;
      { Variable * v;
        ...
        (Reader.LastExpRead->value= _oid_(v));
        Result = v;
        } 
...
{ GC_RESERVE(1);  // HOHO v3.0.55 optim !
  ...
  { char * s2 = GC_STRING(append_string(self,".cl"));
    ...
    { OID gc_local;
      while ((_staritem_star != _oid_(Reader.eof)))
      { GC_LOOP;
        if (b == CTRUE)
         { princ_integer(Reader.reader->nb_line);
          princ_string(":");
...
/* The c++ function for: float!(g0075:string) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
OID  float_I_string_(char *g0075)
{ return _float_(float_I_string(g0075));} 
