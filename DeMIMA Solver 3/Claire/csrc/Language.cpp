...
// definition of the properties 
  ...
  { global_variable * _CL_obj = (Language.LastCall = (global_variable *) Core._global_variable->instantiate("LastCall",Language.it));
  ...
  { global_variable * _CL_obj = (Language.LastComment = (global_variable *) Core._global_variable->instantiate("LastComment",Language.it));
    (_CL_obj->range = Kernel._any);
    (_CL_obj->value = CNULL);
    close_global_variable(_CL_obj);
    } 
  ...
  { (Language._demon = ClaireClass::make("demon",Core._lambda,Language.it));
    CL_ADD_SLOT(Language._demon,demon,Language.pname,pname,Kernel._symbol,_oid_(symbol_I_string2("unamed")));
    CL_ADD_SLOT(Language._demon,demon,Language.priority,priority,Kernel._integer,0);
    CL_ADD_SLOT(Language._demon,demon,Kernel.formula,formula,Core._lambda,CNULL);
    } 
