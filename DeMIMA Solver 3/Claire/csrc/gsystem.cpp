/***** CLAIRE Compilation of file d:\claire\v3.2\src\compile\gsystem.cl 
         [version 3.2.52 / safety 5] Sat Sep 14 18:03:09 2002 *****/
  ...
  { lambda * v7236 = GC_OBJECT(lambda,v7237->formula);
    if (v7236 == (NULL))
     ;else { tformat_string("---- Compiling ~S ---- \n",0,list::alloc(1,_oid_(v7237)));
        ...
        (OBJECT(producer,Generate.PRODUCER->value)->body = v7236->body);
        { ClaireClass * v7243 = Kernel._void;
          OID  v3871;
          { { v7243= check_sort_method(v7237);
              v3871 = OBJECT(producer,Generate.PRODUCER->value)->body;
              } 
            GC_OID(v3871);} 
...
{ GC_BIND;
  ...
  { bag * v11373 = GC_OBJECT(list,parents_list(GC_OBJECT(list,add_modules_list(list::alloc(1,_oid_(v1140))))));
    ...
    if (Optimize.compiler->class2file_ask == CTRUE)
     { tformat_string("**** Compiling the module ~A [v. 3.~A] \n",2,list::alloc(2,_string_(string_I_symbol(v1140->name)),GC_OID(Optimize.compiler->version)));
      write_property(Generate.outmodule,Optimize.OPT,GC_OID(ClAlloc->import(Kernel._port,(int *) fopen_string(append_string(GC_STRING(append_string(GC_STRING(append_string(GC_STRING(Optimize.compiler->source),string_v(Reader._starfs_star->value))),string_v((*Kernel.string_I)(Generate.PRODUCER->value,
        _oid_(v1140->name))))),GC_STRING(OBJECT(producer,Generate.PRODUCER->value)->extension)),"w"))));
    ...
    else { tformat_string("call generate_files ~S  \n",0,list::alloc(1,_oid_(v1140)));
        generate_files_module(v1140);
...
{ GC_BIND;
  tformat_string("==== Generate ~A files for module ~S \n",0,list::alloc(2,GC_OID(_string_(OBJECT(producer,Generate.PRODUCER->value)->comment)),_oid_(v1140)));
  ...
  { OID gc_local;
    ...
    { GC_LOOP;
      { tformat_string("++++ Compiling the file ~A.cl [v. 3.~S] \n",2,list::alloc(2,v7248,GC_OID(Optimize.compiler->version)));
...
{ GC_BIND;
  { ClairePort * v7240 = fopen_string(append_string(GC_STRING(append_string(GC_STRING(append_string(GC_STRING(Optimize.compiler->source),string_v(Reader._starfs_star->value))),string_I_symbol(v7237->name))),GC_STRING(OBJECT(producer,Generate.PRODUCER->value)->extension)),"w");
...
{ GC_BIND;
  ...
  { OID gc_local;
    ...
    { GC_LOOP;
      if (INHERIT(OWNER(v7227),Kernel._class))
       { put_table(Generate.classFile,v7227,ClAlloc->import(Kernel._port,(int *) fopen_string(append_string(GC_STRING(append_string(GC_STRING(append_string(GC_STRING(Optimize.compiler->source),string_v(Reader._starfs_star->value))),string_v((*Kernel.string_I)(Generate.PRODUCER->value,
          GC_OID((*Kernel.name)(v7227)))))),GC_STRING(OBJECT(producer,Generate.PRODUCER->value)->extension)),"w")));
        use_as_output_port(EXPORT((ClairePort *),nth_table1(Generate.classFile,v7227)));
        tformat_string("++++ Creating the file ~A.java\n",2,list::alloc(1,GC_OID((*Kernel.string_I)(Generate.PRODUCER->value,
...
{ GC_BIND;
  { ClairePort * v5173 = fopen_string(append_string(GC_STRING(append_string(GC_STRING(append_string(GC_STRING(Optimize.compiler->source),string_v(Reader._starfs_star->value))),string_v((*Kernel.string_I)(Generate.PRODUCER->value,
      _oid_(v1140->name))))),GC_STRING(OBJECT(producer,Generate.PRODUCER->value)->extension)),"w");
    ...
    claire_gc();
    v5173= fopen_string(append_string(GC_STRING(append_string(GC_STRING(append_string(GC_STRING(Optimize.compiler->source),string_v(Reader._starfs_star->value))),string_v((*Kernel.string_I)(Generate.PRODUCER->value,
      _oid_(v1140->name))))),GC_STRING(OBJECT(producer,Generate.PRODUCER->value)->extension)),"a");
    use_as_output_port(v5173);
    (Optimize.OPT->outfile = v5173);
...
OID  generate_functions_module(module *v1140)
{ GC_BIND;
...
{ GC_RESERVE(1);  // HOHO v3.0.55 optim !
  { ClairePort * v11497 = fopen_string(append_string(v1140,".cl"),"r");
    ...
    if (Optimize.compiler->class2file_ask != CTRUE)
     (Optimize.OPT->outfile = fopen_string(append_string(v11188,GC_STRING(OBJECT(producer,Generate.PRODUCER->value)->extension)),"w"));
...
{ GC_BIND;
  (OBJECT(producer,Generate.PRODUCER->value)->body = v1140->body);
  { OID Result = 0;
    { ClaireClass * v7243 = Kernel._void;
      OID  v3871;
...
{ if ((c_func_any(v3871) == CTRUE) && 
      ((v14719 == CTRUE) && 
        ((v7243 != Kernel._void) && 
          (!INHERIT(OWNER(v3871),Language._If)))))
...
{ GC_BIND;
  { ClaireClass *Result ;
    { ClaireType * v11590 = v1140->range;
      OID  v7226 = GC_OID(OBJECT(producer,Generate.PRODUCER->value)->body);
      ...
      (OBJECT(producer,Generate.PRODUCER->value)->body = c_strict_code_any(v7226,v7243));
      if (_inf_equal_type(v11592,v11590) != CTRUE)
       { tformat_string("---- note: ~S's range was found to be ~S (vs. ~S) \n",2,list::alloc(3,_oid_(v1140),
        ...
        if ((Optimize.compiler->safety < 2) || 
         ...
         (OBJECT(producer,Generate.PRODUCER->value)->body = c_strict_code_any(GC_OID(c_check_any(GC_OID((*Optimize.c_code)(v7226,
        ...
        if (boolean_I_any(sort_equal_class(osort_any(_oid_(v11592)),v7243)) != CTRUE)
         { if ((v7243 != Kernel._void) && 
              ((_oid_((INHERIT(v11592->isa,Kernel._class) ? (ClaireObject *) sort_I_class((ClaireClass *) OBJECT(ClaireClass,_oid_(v11592))) :  (ClaireObject *)  sort_I_type((ClaireType *) OBJECT(ClaireType,_oid_(v11592))))) == _oid_(Kernel._void)) || 
                  (!INHERIT(v7243,OBJECT(ClaireClass,_oid_((INHERIT(v11592->isa,Kernel._class) ? (ClaireObject *) sort_I_class((ClaireClass *) OBJECT(ClaireClass,_oid_(v11592))) :  (ClaireObject *)  sort_I_type((ClaireType *) OBJECT(ClaireType,_oid_(v11592))))))))))
      ...
      { ClaireType * v11593 = GC_OBJECT(ClaireType,OBJECT(ClaireType,(*Kernel._exp)(_oid_(v11590),
        ...
        if ((Optimize.compiler->safety > 1) && 
         ...
         { (OBJECT(producer,Generate.PRODUCER->value)->body = (*Optimize.c_strict_check)(GC_OID(OBJECT(producer,Generate.PRODUCER->value)->body),
      ...
      if ((Optimize.OPT->allocation == CTRUE) && 
          (!BCONTAIN(status_I_restriction(v1140),6)))
...
OID  get_dependents_method(method *v7237)
{ GC_BIND;
  { OID Result = 0;
    { OID gc_local;
      ITERATE(v7240);
...
OID  set_outfile_lambda(lambda *v1140)
{ GC_BIND;
  { OID Result = 0;
      ...
      if (v3109 != CNULL)
       { { ClaireBoolean * g0000I;
          { OID  v13408;
            ...
            g0000I = boolean_I_any(v13408);
            } 
          
          if (g0000I == CTRUE) v11153= string_v((*Kernel.string_I)(Generate.PRODUCER->value,
              _oid_(OBJECT(ClaireClass,v3109)->name)));
            } 
        if (equal_string(v11153,string_I_symbol(ClEnv->module_I->name)) != CTRUE)
         v7240= fopen_string(append_string(GC_STRING(append_string(GC_STRING(append_string(GC_STRING(Optimize.compiler->source),string_v(Reader._starfs_star->value))),v11153)),GC_STRING(OBJECT(producer,Generate.PRODUCER->value)->extension)),"a");
        (Optimize.OPT->outfile = v7240);
        } 
      Result = Kernel.ctrue;
      } 
