...
{int i, n = prototype->length;
 ...
 // <debug for alignment> printf("alloc object size %d -> rep = %d \n",n,rep);
 for (i= 2; i<=n ; i++)
...
{slot *s = (slot *) instantiate(9);
 ...
 int i = slots->length;                  // v3.2: number of existing slots
    ...
    if (c1 == Kernel._float & ix % 2 == CL_FLOAT_PARITY) ix++;     // some architecture requires even float indexes 
...
{ if (c == Kernel._array || c == Kernel._any || c == Kernel._port ||
  ...
  else if (c->superclass == Kernel._import) return c;
