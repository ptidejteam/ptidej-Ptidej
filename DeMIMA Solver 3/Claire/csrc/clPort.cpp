// +--------------------------------------------------------------------------+
// | microCLAIRE C++ Library version 1.00                                     |
// | copyright (C) 1999 Y.Caseau - All Rights Reserved                        |
// | ClairePort.cpp                                                           |
// +--------------------------------------------------------------------------+
...
class CPFile: public ClairePort {
    ...
    virtual void put(double x)
     {double y = floor(x);
         if (x > CLMINFLOAT & x < CLMAXFLOAT & y == x) fprintf(value,"%.1f",x);
         else if (y == 0.0 || ( (x - y) / y > 1e-6 && DECIMAL(x)))         // v3.2.5
             fprintf(value,"%g",x);               // easy print is OK
        else fprintf(value,"%0.20g",x);}          // v3.2.24
   ...
   public:
    ...
    virtual void put(double x)
       {double y = floor(x);
        ...
        else sprintf(&buffer[index],"%0.20g",x);
        for ( ;buffer[index] != '\0'; index++) ;
...
{int x = 0;
  if (p->rdepth == 0) {msec(x); p->rstart = x;}
  p->rdepth++; p->rnum++;
  return p;}
...
{int x = 0;
  p->rdepth--;
  if (p->rdepth == 0) {msec(x); p->rtime += (x - p->rstart);}}
