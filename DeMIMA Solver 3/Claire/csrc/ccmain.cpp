/***** CLAIRE Compilation of file ccmain.cl 
         [version 3.2.52 / safety 5] Thu Feb 13 21:07:04 2003 *****/
...
{ GC_BIND;
  { ClaireBoolean * rCode = CTRUE;
    ...
    int  vlevel = 2;
    list * l = ((list *) check_in_bag(copy_bag(lp),Kernel._list,Kernel._string));
    { ClaireHandler c_handle = ClaireHandler();
      if ERROR_IN 
      { { (Reader._starfs_star->value= _string_("\\"));
          (OBJECT(producer,Generate.PRODUCER->value)->extension = ".cpp");
          update_property(Optimize.libraries_dir,
            Optimize.compiler,
            17,
            Kernel._object,
            _oid_(list::alloc(Kernel._any,3,_string_("C:\\Docume~1\\Yann\\Work\\Ptidej~2\\Claire\\bin\\public\\ntv"),
              _string_("C:\\Docume~1\\Yann\\Work\\Ptidej~2\\Claire\\bin\\debug\\ntv"),
              _string_("C:\\Docume~1\\Yann\\Work\\Ptidej~2\\Claire\\bin\\public\\ntv"))));
          (Optimize.compiler->headers_dir = "C:\\Docume~1\\Yann\\Work\\Ptidej~2\\Claire\\include");
          update_property(Optimize.options,
            Optimize.compiler,
            19,
            Kernel._object,
            _oid_(list::alloc(Kernel._any,3,_string_("-c /O2"),
              _string_("-c /Zi"),
              _string_("-c"))));
          ...
          if (equal_string(_Zout,"") == CTRUE)
           { if (equal_string(_Zcm,"") != CTRUE)
             ...
             equal(_string_(_Zout),_string_(_Zcf));
            } 
          if (_Zinit_ask == CTRUE)
           load_string("init");
...
{ GC_BIND;
  { list * l = GC_OBJECT(list,add_modules_list(GC_OBJECT(list,OBJECT(list,Optimize.claire_modules->value))));
    ...
    if (equal_string(out,"") != CTRUE)
     generate_system_file_string(f,l);
