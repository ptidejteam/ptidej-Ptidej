/***** CLAIRE Compilation of file d:\claire\v3.2\src\meta\object.cl 
         [version 3.2.52 / safety 5] Sat Sep 14 18:02:42 2002 *****/
...
// recommendation
/* The c++ function for: about(_CL_obj:void) [NEW_ALLOC] */
OID  about_void()
{ GC_BIND;
  princ_string("CLAIRE v3.");
  print_any(_float_(ClEnv->version));
  princ_string(" Copyright (C) 1994-2001 Yves Caseau. All Rights Reserved.\n");
  ...
  { int  i = ClEnv->index;
    ...
    if (get_table(Core.StopProperty,_oid_(prop)) != CNULL)
     { { ClaireBoolean * g0037I;
          ...
          { v_or = ((equal(nth_table1(Core.StopProperty,_oid_(prop)),Core.nil->value) == CTRUE) ? CTRUE : CFALSE);
            if (v_or == CTRUE) g0037I =CTRUE; 
                ...
                { OID gc_local;
                  ...
                  { GC_LOOP;
                    { ClaireBoolean * g0039I;
                      { OID  g0040UU;
                        { int  j = 1;
                          int  g0036 = (*Kernel.length)(l2);
                          { OID gc_local;
                            g0040UU= _oid_(CFALSE);
                            while ((j <= g0036))
                            { GC_LOOP;
                              if (((j+start) > i) || 
                                  (equal(GC_OID((*Kernel.nth)(l2,
                                    j)),GC_OID(ClEnv->stack[((start+j)-1)])) != CTRUE))
                        ...
                        g0039I = not_any(g0040UU);
                        } 
                      
                      if (g0039I == CTRUE) { g0038UU = Kernel.ctrue;
                          break;} 
                        } 
                    GC_UNLOOP;} 
                  } 
                v_or = boolean_I_any(g0038UU);
                } 
              if (v_or == CTRUE) g0037I =CTRUE; 
              else g0037I = CFALSE;} 
            } 
        ...
        if (g0037I == CTRUE) close_exception(((general_error *) (*Core._general_error)(_string_("stop as required in ~S(~A)"),
  ...
  { int  v = ClEnv->debug_I;
    if (v > 0)
     { if (n != 0)
      ...
      if (equal(self->if_write,Core.nil->value) == CTRUE)
       { OID  mtest = GC_OID(_oid_(ClEnv->spy_I));
        if (mtest != CNULL)
         { ClaireObject * m = OBJECT(ClaireObject,mtest);
          (ClEnv->spy_I = NULL);
    ...
    { OID  p = a->params;
      ...
      { OID  v;
        if (INHERIT(OWNER(p),Kernel._integer))
         ...
         Result = v;
        else close_exception(((general_error *) (*Core._general_error)(_string_("[138] the value ~S(~S) is unknown !"),
...
{ if ((a->if_write != CNULL) && 
  ...
  { fastcall_relation2(a,x,y);
    ...
    { { OID  r = GC_OID(get_property(Kernel.inverse,a));
        ...
        if (boolean_I_any(old) == CTRUE)
         { OID  g0041UU;
          if (OBJECT(set,y)->length == 0)
           g0041UU = y;
          else if (a->multivalued_ask == Kernel._list)
           g0041UU = CNULL;
          else g0041UU = _oid_(Kernel.emptySet);
            ((*(((list *) a->graph)))[get_index_table1(a,x)]=g0041UU);
          } 
        if (r != CNULL)
         { OID gc_local;
          ITERATE(z);
...
{ { OID Result = 0;
    { OID  p = a->params;
      ...
      if (INHERIT(OWNER(p),Kernel._list))
       { if ((belong_to(x,(*(((bag *) a->domain)))[1]) != CTRUE) || 
            (belong_to(y,(*(((bag *) a->domain)))[2]) != CTRUE))
         close_exception(((general_error *) (*Core._general_error)(_string_("[135] ~S does not belong to the domain of ~S"),
          _oid_(list::alloc(2,x,_oid_(a))))));
        ...
        if ((v != CNULL) || 
          (belong_to(v,_oid_(a->range)) == CTRUE))
       Result = v;
      else close_exception(((general_error *) (*Core._general_error)(_string_("~S(~S) is unknown !"),
          _oid_(list::alloc(2,_oid_(a),x)))));
        } 
    return (Result);} 
  } 
      ...
      { OID  r2 = GC_OID(get_property(Kernel.inverse,r));
        if (INHERIT(OWNER(r2),Kernel._table))
         { OID  v = nth_table1(OBJECT(table,r2),x);
          if (boolean_I_any(_oid_(OBJECT(ClaireRelation,r2)->multivalued_ask)) == CTRUE)
           V_CC = OBJECT(bag,v);
         ...
         { if (boolean_I_any(_oid_(r->multivalued_ask)) == CTRUE)
...
{ GC_BIND;
  { int  n = self->index;
    ...
    { char * g0044UU;
      if (n == 1)
       g0044UU = "dynamic allocation, item is too big (~S)";
      else if (n == 2)
       g0044UU = "dynamic allocation, too large for available memory (~S)";
      else if (n == 3)
       g0044UU = "object allocation, too large for available memory (~S)";
      else if (n == 5)
       g0044UU = "nth[~S] outside of scope for ~S";
      else if (n == 7)
       g0044UU = "Skip applied on ~S with a negative argument ~S";
      else if (n == 8)
       g0044UU = "List operation: cdr(()) is undefined";
      else if (n == 9)
       g0044UU = "String buffer is full: ~S";
      else if (n == 10)
       g0044UU = "Cannot create an imported entity from NULL reference";
      else if (n == 11)
       g0044UU = "nth_string[~S]: string too short~S";
      else if (n == 12)
       g0044UU = "Symbol Table table full";
      else if (n == 13)
       g0044UU = "Cannot create a subclass for ~S [~A]";
      else if (n == 16)
       g0044UU = "Temporary output string buffer too small";
      else if (n == 17)
       g0044UU = "Bag Type Error: ~S cannot be added to ~S";
      else if (n == 18)
       g0044UU = "definition of ~S is in conflict with an object from ~S";
      else if (n == 19)
       g0044UU = "Integer overflow";
      else if (n == 20)
       g0044UU = "Integer arithmetic: division/modulo of ~A by 0";
      else if (n == 21)
       g0044UU = "Integer to character: ~S is a wrong value";
      else if (n == 22)
       g0044UU = "Cannote create a string with negative length ~S";
      else if (n == 23)
       g0044UU = "Not enough memory to instal claire";
      else if (n == 24)
       g0044UU = "execution stack is full [~A]";
      else if (n == 26)
       g0044UU = "Wrong usage of time counter [~A]";
      else if (n == 27)
       g0044UU = "internal garbage protection stack overflow";
      else if (n == 28)
       g0044UU = "the multivalued status of ~S is not compatible with ~S";
      else if (n == 29)
       g0044UU = "There is no module ~S";
      else if (n == 30)
       g0044UU = "Attempt to read a private symbol ~S";
      else if (n == 31)
       g0044UU = "External function not compiled yet";
      else if (n == 32)
       g0044UU = "Too many arguments (~S) for function ~S";
      else if (n == 33)
       g0044UU = "Exception handling: stack overflow";
      else if (n == 34)
       g0044UU = "User interrupt: EXECUTION ABORTED";
      else if (n == 35)
       g0044UU = "reading char '~S': wrong char: ~S";
      else if (n == 36)
       g0044UU = "cannot open file ~A";
      else if (n == 37)
       g0044UU = "world stack is full";
      else if (n == 38)
       g0044UU = "Undefined access to ~S";
      else if (n == 39)
       g0044UU = "cannot convert ~S to an integer";
      else if (n == 40)
       g0044UU = "integer multiplication overflow with ~S and ~S";
      else if (n == 41)
       g0044UU = "wrong NTH access on ~S and ~S";
      else if (n == 42)
       g0044UU = "Wrong array[~S] init value: ~S";
      else if (n == 50)
       g0044UU = "C++ imported error (~S) : ~S";
      else { (self->value = n);
          g0044UU = "What the hell is this ! [code: ~S^]";
          } 
        format_string(g0044UU,list::alloc(2,GC_OID(self->value),GC_OID(self->arg)));
      } 
    } 
  GC_UNBIND;} 
...
{ if ((self->value != CNULL) && 
      ((Kernel._set != self->range->isa) && 
          (belong_to(self->value,_oid_(self->range)) != CTRUE)))
