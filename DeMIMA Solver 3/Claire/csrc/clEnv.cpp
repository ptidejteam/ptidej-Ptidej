...
{ ClEnv->cout = ClAlloc->stdErr;
  if (n == -2) {princ_string("Assertion violation in file "); princ_string((char *) a);
                princ_string(" [line "); princ_integer(b); princ_string(" \n");
                ClEnv->abort();}
  else if (n == -1) { princ_string("System Error: "); princ_string((char *) a);
                      princ_string(" \n");
                      ClEnv->abort();}
