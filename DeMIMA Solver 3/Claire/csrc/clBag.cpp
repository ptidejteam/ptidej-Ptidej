...
{if (l->of != NULL && l->of->contains(val) == CFALSE )
...
{  list *obj = list::make();
   ...
   OID *x = ClAlloc->makeContent(l->length + 1);
     ...
     obj->content = x;
     if (l->of != NULL && l->of->contains(val) == CFALSE ) obj->of = Kernel._any;
...
{int i,j,m = l->length;
 if (l->of != NULL && l->of->contains(val) == CFALSE ) Cerror(17,val,_oid_(l)); // v3.2
...
{// see(">>>> call add_set on ",_oid_(l));
 // see("---- of is ", _oid_(l->of));
  if (l->of != NULL && l->of->contains(val) == CFALSE ) Cerror(17,val,_oid_(l)); // v3.2
  return l->addFast(val);}
...
// creates a temporary list on the stack
...
{va_list ap;
 ...
 tuple *obj = (tuple *) adr;
   ...
   obj->length = n;
   obj->content = &adr[4];
   (*obj)[0] =  // ClAlloc->log2up(n);
                ClaireAllocation::log2up(n);        // v3.2.38
   ClEnv->index += (n + 3);
   if (ClEnv->index >= ClAlloc->maxStack) Cerror(27,ClEnv->index,0);
