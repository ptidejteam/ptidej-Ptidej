/***** CLAIRE Compilation of file d:\claire\v3.2\src\meta\method.cl 
      ...
      if (Kernel._slot == s->isa)
       { OID  z = slot_get_object(x,CLREAD(slot,s,index),CLREAD(slot,s,srange));
        if ((z != CNULL) || 
         ...
         Result = z;
        else close_exception(((read_slot_error *) (*Core._read_slot_error)(_oid_(x),
            _oid_(self))));
          } 
      else close_exception(((read_slot_error *) (*Core._read_slot_error)(_oid_(x),
          _oid_(self))));
...
{ return (_void_(close_exception(((range_error *) (*Core._range_error)(_oid_(self),
    y,
    _oid_(self->range))))));} 
      ...
      if (boolean_I_any(_oid_(s)) != CTRUE)
       close_exception(((selector_error *) (*Core._selector_error)(_oid_(self),
        _oid_(list::alloc(1,_oid_(x))))));
          ...
          if (boolean_I_any(_oid_(self->multivalued_ask)) == CTRUE)
           { { ClaireRelation * r = self->inverse;
              if (r == (NULL))
               ;else { OID gc_local;
                  ITERATE(y1);
            ...
            { OID  l;
              { { OID  g0000UU;
                  { if (self->store_ask == CTRUE)
                     g0000UU = (*Kernel.copy)(y);
                    else g0000UU = y;
                      GC_OID(g0000UU);} 
                  l = (*Kernel.shrink)(g0000UU,
                    0);
                  } 
                GC_OID(l);} 
              store_object(x,
...
{ if (ClEnv->verbose == 4)
   tformat_string("insert ~S into ~S \n",0,list::alloc(2,_oid_(p),_oid_(r)));
...
{ { list *Result ;
    { property * p = x->selector;
      ...
      if (boolean_I_any(_oid_(p->dictionary)) == CTRUE)
       { if (uniform_restriction(x) == CTRUE)
         hashinsert_restriction(x);
        else (p->dictionary = CFALSE);
          } 
      if ((ix > 0) && 
          ((Kernel._method == x->isa) && (CLREAD(method,x,evaluate) != (NULL))))
                       ...
                       { { ClaireBoolean * g0003I;
                          { OID  g0004UU;
                            { ClaireClass * c3 = domain_I_restriction(OBJECT(restriction,y));
                              g0004UU = _oid_(not_any(_oid_(((INHERIT(OBJECT(ClaireClass,c2),c3)) ? ((INHERIT(c3,c)) ? CTRUE: CFALSE): CFALSE))));
                              } 
                            g0003I = not_any(g0004UU);
                            } 
                          
                          if (g0003I == CTRUE) { g0002UU = Kernel.ctrue;
                              break;} 
                            } 
                        } 
                      GC_UNLOOP;} 
                    } 
                  g0001I = not_any(g0002UU);
                  } 
                
                if (g0001I == CTRUE) ((*(((list *) OBJECT(ClaireClass,c2)->dispatcher)))[ix]=_oid_(CLREAD(method,x,evaluate)));
                  } 
              } 
            GC_UNLOOP;} 
          } 
          ...
          { ClaireBoolean * g0007I;
            { OID  g0008UU;
              { list * l2 = OBJECT(restriction,r)->domain;
                { ClaireBoolean * V_CL0009;{ ClaireBoolean *v_and;
                    { v_and = inherit_ask_class(OWNER((*(l))[1]),Kernel._class);
                      if (v_and == CFALSE) V_CL0009 =CFALSE; 
                      else { v_and = ((l2->length == n) ? CTRUE : CFALSE);
                        if (v_and == CFALSE) V_CL0009 =CFALSE; 
                        else { { OID  g0010UU;
                            { int  i = 2;
                              int  g0005 = n;
                              { g0010UU= _oid_(CFALSE);
                                while ((i <= g0005))
                                { if (equal((*(l))[i],(*(l2))[i]) != CTRUE)
                                   { g0010UU = Kernel.ctrue;
                                    break;} 
                                  ++i;
                                  } 
                                } 
                              } 
                            v_and = not_any(g0010UU);
                            } 
                          if (v_and == CFALSE) V_CL0009 =CFALSE; 
                          else V_CL0009 = CTRUE;} 
                        } 
                  ...
                  g0008UU=_oid_(V_CL0009);} 
                } 
              g0007I = not_any(g0008UU);
              } 
            
            if (g0007I == CTRUE) { g0006UU = Kernel.ctrue;
                break;} 
              } 
          } 
        Result = not_any(g0006UU);
        } 
      } 
    return (Result);} 
  } 
    ...
    { ClaireBoolean *v_and;
      { { OID  g0011UU;
          { ITERATE(x);
            ...
            if (Kernel._method != OBJECT(ClaireObject,x)->isa)
             { g0011UU = Kernel.ctrue;
              break;} 
            } 
          v_and = not_any(g0011UU);
          } 
...
{ if (boolean_I_any(_oid_(c->dictionary)) != CTRUE)
  ...
  return (hashinsert_list(c->dictionary,x));} 
        ...
        { GC_LOOP;
          if (((*(l))[i] == CNULL) || 
           ...
           { if ((hashsize_list(l)*3) > (l->length*2))
             { list * myl2;
              { { list * g0013 = l;
                  list * g0014 = GC_OBJECT(list,make_list_integer((((*(g0013))[0])*2),CNULL));
                  { OID gc_local;
                    ITERATE(g0015);
                    for (START(g0013); NEXT(g0015);)
                    { GC_LOOP;
                      if (g0015 != CNULL)
                       hashinsert_list(g0014,OBJECT(method,g0015));
                      GC_UNLOOP;} 
                    } 
                  myl2 = g0014;
                  } 
...
{ if (boolean_I_any(_oid_(self->dictionary)) == CTRUE) 
  { { ClaireObject *Result ;
    ...
    { int  x = (m-n);
      ...
      if ((z == x) && 
       ...
       { OID  g0022UU;
        { int  i = 1;
          int  g0020 = x;
          { g0022UU= _oid_(CFALSE);
            while ((i <= g0020))
            { { int  y = ((n-1)+i);
                OID  u = (*(l))[i];
                if ((OWNER(u) == Kernel._class) ? (!INHERIT(OWNER(ClEnv->stack[y]),OBJECT(ClaireClass,u))) : (vmatch_ask_any(u,ClEnv->stack[y],n) != CTRUE))
                 { g0022UU = Kernel.ctrue;
                  break;} 
                } 
              ++i;
              } 
     ...
     { ClaireBoolean *v_and;
      { v_and = vmatch_ask_any(_oid_(OBJECT(Param,t)->arg),x,n);
        if (v_and == CFALSE) Result =CFALSE; 
            ...
            { int  i = 1;
              int  g0024 = OBJECT(Param,t)->params->length;
                    ...
                    { OID  g0028UU;
                      { OID  _Zt = (*(OBJECT(Param,t)->args))[i];
                        ...
                        if (Kernel._set == OWNER(_Zt))
                         g0028UU = _oid_(Ztype_any(_Zv,_Zt));
                        else g0028UU = _oid_(vmatch_ask_any(_Zt,_Zv,n));
                          } 
                      g0027I = not_any(g0028UU);
                      } 
                    
                    if (g0027I == CTRUE) { g0026UU = Kernel.ctrue;
                        break;} 
                      } 
                  ++i;
                  } 
     ...
     { ClaireBoolean *v_and;
      { v_and = tmatch_ask_any(t,_oid_(OBJECT(Param,t2)->arg),l);
        if (v_and == CFALSE) Result =CFALSE; 
            ...
            { int  i = 1;
              int  g0032 = OBJECT(Param,t2)->params->length;
              { OID gc_local;
       ...
       { ClaireBoolean *v_and;
        { v_and = ((OBJECT(bag,t2)->length == OBJECT(bag,t)->length) ? CTRUE : CFALSE);
          if (v_and == CFALSE) Result =CFALSE; 
              ...
              { int  i = 1;
                int  g0033 = OBJECT(bag,t2)->length;
                { OID gc_local;
