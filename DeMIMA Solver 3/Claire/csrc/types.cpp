/***** CLAIRE Compilation of file d:\claire\v3.2\src\meta\types.cl 
...
{ { ClaireBoolean *Result ;
    if (Kernel._set == self->isa)
     ...
     { OID  g0072UU;
      { ITERATE(t);
        g0072UU= _oid_(CFALSE);
        for (START(((bag *) self)); NEXT(t);)
        if (not_any((*Core.finite_ask)(t)) == CTRUE)
         { g0072UU = Kernel.ctrue;
          break;} 
        else ;} 
      Result = not_any(g0072UU);
      } 
...
{ GC_BIND;
  { ClaireType *Result ;
    { ClaireObject *V_CC ;
      ...
      else if (!INHERIT(self,Kernel._type))
...
{ { ClaireBoolean *Result ;
    { OID  g0075UU;
      { ITERATE(x);
        ...
        if (not_any((*Core.finite_ask)(x)) == CTRUE)
         { g0075UU = Kernel.ctrue;
          break;} 
        } 
      Result = not_any(g0075UU);
      } 
    return (Result);} 
  } 
        ...
        { ClaireBoolean *v_and;
          { v_and = _Z_any1(x,CLREAD(Param,y,arg));
            if (v_and == CFALSE) V_CC =CFALSE; 
                ...
                { ClaireBoolean * V_CL0082;{ OID  g0083UU;
                    { OID gc_local;
                      ITERATE(p);
                      g0083UU= _oid_(CFALSE);
                      bag *p_support;
                      ...
                      { GC_LOOP;
                        if (Ztype_any(GC_OID(funcall_property(OBJECT(property,p),x)),(*(l))[n]) != CTRUE)
                         { g0083UU = Kernel.ctrue;
                          break;} 
                        else ++n;
                          GC_UNLOOP;} 
                      } 
                    V_CL0082 = boolean_I_any(g0083UU);
                    } 
                  
                  g0081UU=_oid_(V_CL0082);} 
                v_and = ((g0081UU != Kernel.ctrue) ? CTRUE : CFALSE);
                } 
              if (v_and == CFALSE) V_CC =CFALSE; 
              else V_CC = CTRUE;} 
            } 
          ...
          { v_and = ((OBJECT(bag,x)->length == n) ? CTRUE : CFALSE);
            if (v_and == CFALSE) V_CC =CFALSE; 
                ...
                { int  i = 1;
                  int  g0080 = n;
                  { OID gc_local;
                    g0084UU= _oid_(CFALSE);
                    while ((i <= g0080))
                    { // HOHO, GC_LOOP not needed !
                      if (belong_to((*(OBJECT(bag,x)))[i],(*(((bag *) y)))[i]) != CTRUE)
                       { g0084UU = Kernel.ctrue;
                        break;} 
                      ++i;
                      } 
                    } 
                  } 
                v_and = not_any(g0084UU);
                } 
              if (v_and == CFALSE) V_CC =CFALSE; 
              else V_CC = CTRUE;} 
            } 
    ...
    { { ClaireType *Result ;
        { list * g0085UU;
          { { bag *v_list; OID v_val;
              OID z,CLcount;
              v_list = x->subclass;
               g0085UU = v_list->clone();
              for (CLcount= 1; CLcount <= v_list->length; CLcount++)
              { z = (*(v_list))[CLcount];
                v_val = (*Core.glb)(z,
                  _oid_(y));
                
                (*((list *) g0085UU))[CLcount] = v_val;} 
              } 
            GC_OBJECT(list,g0085UU);} 
          Result = Uall_list(g0085UU);
          } 
        return (Result);} 
       } 
    else{ GC_BIND;
  ...
  { ClaireType *Result ;
    if (INHERIT(y->isa,Core._Param))
     { ClaireType * c = join_class(x->arg,CLREAD(Param,y,arg));
      ...
      { OID gc_local;
        ...
        { GC_LOOP;
          { ClaireType * t = GC_OBJECT(ClaireType,OBJECT(ClaireType,(*Core.glb)(GC_OID(_oid_(_at_type(x,OBJECT(property,p)))),
            ...
            if (boolean_I_any(_oid_(t)) == CTRUE)
             l= l->addFast(_oid_(t));
      ...
      if (boolean_I_any(_oid_(c)) == CTRUE)
       { Param * _CL_obj = ((Param *) GC_OBJECT(Param,new_object_class(Core._Param)));
...
{ GC_BIND;
  { ClaireType *Result ;
    if (INHERIT(y->isa,Kernel._class))
     { if (boolean_I_any(_oid_(join_class(x->arg,((ClaireClass *) y)))) == CTRUE)
       Result = nth_class1(((ClaireClass *) join_class(x->arg,((ClaireClass *) y))),GC_OBJECT(ClaireType,x->t1));
      else Result = Kernel.emptySet;
        } 
    else if (INHERIT(y->isa,Core._Param))
     { if (boolean_I_any(_oid_(join_class(x->arg,CLREAD(Param,y,arg)))) == CTRUE)
     ...
     { if (boolean_I_any(_oid_(join_class(x->arg,CLREAD(subtype,y,arg)))) == CTRUE)
       { OID  t = GC_OID((*Core.glb)(GC_OID(_oid_(x->t1)),
          GC_OID(_oid_(CLREAD(subtype,y,t1)))));
        if (boolean_I_any(t) == CTRUE)
     ...
     { if (CLREAD(subtype,y,arg) == Kernel._tuple)
       { list * g0086UU;
        { { bag *v_list; OID v_val;
            OID z,CLcount;
            v_list = x;
             g0086UU = v_list->clone();
            for (CLcount= 1; CLcount <= v_list->length; CLcount++)
            { z = (*(v_list))[CLcount];
              v_val = (*Core.glb)(z,
                GC_OID(_oid_(CLREAD(subtype,y,t1))));
              
              (*((list *) g0086UU))[CLcount] = v_val;} 
            } 
          GC_OBJECT(list,g0086UU);} 
        Result = tuple_I_list(g0086UU);
        } 
      else Result = Kernel.emptySet;
        } 
    else Result = OBJECT(ClaireType,(*Core.glb)(_oid_(y),
        ...
        { OID gc_local;
          ...
          { GC_LOOP;
            { ClaireType * z = GC_OBJECT(ClaireType,OBJECT(ClaireType,(*Core.glb)((*(x))[i],
                (*(y))[i])));
              if (boolean_I_any(_oid_(z)) == CTRUE)
               r= r->addFast(_oid_(z));
              else { r= Kernel.nil;
                  { ;break;} 
                  } 
    ...
    { ClaireType * z = of_bag(s);
      if (equal(_oid_(z),_oid_(Kernel.emptySet)) != CTRUE)
       Result = OBJECT(ClaireBoolean,_oid_((ClaireObject *) Core._inf_equalt->fcall(((int) z),((int) y))));
          ...
          { ITERATE(x);
            g0088UU= _oid_(CFALSE);
            for (START(s); NEXT(x);)
            if (Ztype_any(x,_oid_(y)) != CTRUE)
             { g0088UU = Kernel.ctrue;
              break;} 
            } 
          Result = not_any(g0088UU);
          } 
        } 
    return (Result);} 
  } 
          ...
          if (v_and == CFALSE) Result =CFALSE; 
              ...
              { int  i = 1;
                ...
                { OID gc_local;
                  ...
                  while ((i <= g0091))
                  { // HOHO, GC_LOOP not needed !
                    if (not_any((*Core._inf_equalt)((*(x))[i],
                      (*(((bag *) y)))[i])) == CTRUE)
     ...
     { ClaireBoolean *v_and;
      { v_and = OBJECT(ClaireBoolean,_oid_((ClaireObject *) Core._inf_equalt->fcall(((int) x),((int) CLREAD(Param,y,arg)))));
        if (v_and == CFALSE) Result =CFALSE; 
            ...
            { int  n = 1;
              int  g0094 = CLREAD(Param,y,params)->length;
              { OID gc_local;
                g0095UU= _oid_(CFALSE);
                while ((n <= g0094))
                { GC_LOOP;
                  if (not_any((*Core._inf_equalt)(GC_OID(_oid_(_at_type(x,OBJECT(property,(*(CLREAD(Param,y,params)))[n])))),
                    (*(CLREAD(Param,y,args)))[n])) == CTRUE)
                   { g0095UU = Kernel.ctrue;
                    break;} 
                  ++n;
                  GC_UNLOOP;} 
                } 
              } 
            v_and = not_any(g0095UU);
            } 
          if (v_and == CFALSE) Result =CFALSE; 
          else Result = CTRUE;} 
        } 
     ...
     { list * g0096UU;
      { { bag *v_list; OID v_val;
          ...
          { y = (*(v_list))[CLcount];
            ...
            (*((list *) g0096UU))[CLcount] = v_val;} 
          } 
        GC_OBJECT(list,g0096UU);} 
      Result = Uall_list(g0096UU);
      } 
    else Result = Kernel.emptySet;
      GC_UNBIND; return (Result);} 
  } 
  ...
  { ClaireType *Result ;
    if (INHERIT(x->isa,Kernel._class))
     { ClaireObject * r = GC_OBJECT(ClaireObject,_at_property1(p,((ClaireClass *) x)));
      if (boolean_I_any(_oid_(r)) == CTRUE)
       Result = OBJECT(ClaireType,(*Kernel.range)(_oid_(r)));
      else Result = Kernel._any;
        } 
    else if (INHERIT(x->isa,Core._Param))
     ...
     { list * g0097UU;
      { { bag *v_list; OID v_val;
          OID y,CLcount;
          v_list = ((bag *) x);
           g0097UU = v_list->clone();
          for (CLcount= 1; CLcount <= v_list->length; CLcount++)
          { y = (*(v_list))[CLcount];
            v_val = _oid_(set::alloc(1,GC_OID(funcall_property(p,y))));
            
            (*((list *) g0097UU))[CLcount] = v_val;} 
          } 
        GC_OBJECT(list,g0097UU);} 
      Result = Uall_list(g0097UU);
      } 
...
/* The c++ function for: nth_get_array_type */
ClaireType * nth_get_array_type(ClaireType *a,ClaireType *n)
...
{ { OID Result = 0;
    ...
    else close_exception(((general_error *) (*Core._general_error)(_string_("[180] nth[~S] out of scope for ~S"),
