/***** CLAIRE Compilation of file d:\claire\v3.2\src\compile\cgen.cl 
...
#include <Optimize.h>
#include <Generate.h>
char * string_I_c_producer1(c_producer *v7227,Variable *v1140)
...
char * string_I_c_producer2(c_producer *v7227,symbol *v1140)
{ print_in_string_void();
  ident_c_producer(v7227,v1140);
  return (end_of_print_void());} 

void  ident_c_producer3(c_producer *v7227,Variable *v7247)
{ if (Optimize.compiler->naming == 2) 
...
void  ident_c_producer(c_producer *v7227,symbol *v7243)
{ { int  v7239 = index_list(v7227->bad_names,_oid_(v7243));
    if (v7239 == 0)
     c_princ_string(string_I_symbol(v7243));
    else c_princ_symbol(OBJECT(symbol,(*(v7227->good_names))[v7239]));
...
void  produce_c_producer2(c_producer *v7227,OID v7248)
{ GC_BIND;
  if (INHERIT(OWNER(v7248),Kernel._function))
   ...
   { if ((equal(_oid_(OBJECT(global_variable,v7248)->range),_oid_(Kernel.emptySet)) == CTRUE) && 
        (INHERIT(OWNER(OBJECT(global_variable,v7248)->value),Kernel._integer)))
     (*Kernel.princ)(GC_OID(OBJECT(global_variable,v7248)->value));
   ...
   { ident_c_producer(v7227,defined_symbol(OBJECT(ClaireClass,v7248)->name)->name);
    princ_string("._");
...
void  stat_exp_c_producer(c_producer *v7227,OID v1140,OID v15308)
{ if (INHERIT(OWNER(v1140),Optimize._to_C)) 
  { stat_exp_c_producer(v7227,GC_OID(OBJECT(to_C,v1140)->arg),v15308);
     } 
  else{ if (INHERIT(OWNER(v1140),Optimize._to_CL)) 
    { stat_exp_c_producer(v7227,GC_OID(OBJECT(to_CL,v1140)->arg),v15308);
       } 
...
void  namespace_I_c_producer(c_producer *v7227,module *v7237)
{ princ_string("\n\n// namespace class for ");
...
void  module_I_c_producer(c_producer *v7227,module *v7237)
{ princ_string("\n\n// module definition \n");
...
void  declare_c_producer(c_producer *v7227,property *v7240)
{ breakline_void();
...
void  start_module_interface_c_producer(c_producer *v7227,module *v1140)
{ GC_BIND;
...
void  end_module_interface_c_producer(c_producer *v7227,module *v1140)
{ princ_string("\n#endif\n");
  } 

void  generate_end_file_c_producer(c_producer *v7234,module *v7237)
{ GC_BIND;
  close_block_void();
  ...
  { OID gc_local;
    ...
    { GC_LOOP;
      if (INHERIT(OWNER(v7227),Kernel._class))
       { ClairePort * v7240 = fopen_string(append_string(GC_STRING(append_string(GC_STRING(append_string(GC_STRING(Optimize.compiler->source),string_v(Reader._starfs_star->value))),string_I_c_producer2(v7234,OBJECT(symbol,(*Kernel.name)(v7227))))),GC_STRING(v7234->extension)),"a");
...
void  generate_classes_c_producer(c_producer *v7234,char *v7243,module *v7237)
{ princ_string("\n\n");
  princ_string(v7243);
  princ_string("Class ");
  princ_string(v7243);
...
void  generate_start_file_c_producer(c_producer *v7227,module *v7237)
{ tformat_string("++++ Creating the file ~A.cpp \n",2,list::alloc(1,_string_(string_I_c_producer2(v7227,v7237->name))));
...
void  generate_meta_load_c_producer(c_producer *v7227,module *v7237)
{ princ_string("void ");
  ident_c_producer(v7227,v7237->name);
...
void  define_variable_c_producer2(c_producer *v7227,ClaireClass *v7244,char *v7247)
...
void  generate_profile_c_producer(c_producer *v7227,OID v7237)
...
void  generate_interface_c_producer(c_producer *v7227,module *v1140)
    ...
    { GC_LOOP;
      if (INHERIT(OWNER(v7248),Kernel._class))
       { princ_string("\n\nclass ");
        ident_symbol(OBJECT(symbol,(*Kernel.name)(v7248)));
        princ_string(": public ");
        ident_symbol(OBJECT(ClaireClass,v7248)->superclass->name);
        new_block_void();
        breakline_void();
        princ_string("public:");
        { OID gc_local;
            ...
            { ClaireBoolean * g0042I;
              { OID  v3422;
                { OID gc_local;
                  ...
                  { GC_LOOP;
                    if (OBJECT(restriction,v11592)->selector == OBJECT(restriction,v7249)->selector)
                     { v3422 = Kernel.ctrue;
                      break;} 
                    GC_UNLOOP;} 
                  } 
                g0042I = not_any(v3422);
                } 
              
              if (g0042I == CTRUE) { breakline_void();
                  princ_string("   ");
...
void  gc_introduction_c_producer(c_producer *v7227,OID v9221)
  ...
  { OID Result = 0;
    if (INHERIT(OWNER(v1140),Language._Instruction))
     { if (INHERIT(OBJECT(ClaireObject,v1140)->isa,Language._Construct))
       ...
       Result = gc_usage_any(GC_OID(OBJECT(to_C,v1140)->arg),v15308);
      else if (INHERIT(OBJECT(ClaireObject,v1140)->isa,Optimize._to_CL))
       Result = gc_usage_any(GC_OID(OBJECT(to_CL,v1140)->arg),v15308);
...
void  debug_intro_c_producer(c_producer *v7227,lambda *v1140,method *v7248)
{ GC_BIND;
  { module * v7237;
    { ClaireObject *V_CC ;
...
char * protect_result_c_producer(c_producer *v7227,ClaireClass *v7243,ClaireBoolean *v13756,OID v7248)
{ GC_BIND;
...
void  generate_function_start_c_producer(c_producer *v7227,lambda *v1140,ClaireClass *v7243,OID v7237,char *v12719)
{ GC_BIND;
  { OID  v3109;
...
void  generate_regular_function_c_producer(c_producer *v7227,lambda *v1140,ClaireFunction *v9226,ClaireClass *v7243,OID v7237,list *v11442)
{ GC_BIND;
  if (Optimize.compiler->naming != 2)
   { princ_string("\n/* The c++ function for: ");
    if (Kernel._method == OWNER(v7237))
...
void  generate_float_function_c_producer(c_producer *v7227,method *v7237,char *v12719)
{ GC_BIND;
          ...
          { Variable * v2072 = ((Variable *) GC_OBJECT(Variable,new_object_class(Language._Variable)));
            ...
            { Variable * v6723; 
              ...
              if (v7248 == _oid_(Kernel._float))
               v6724 = Kernel._any;
              else v6724 = OBJECT(ClaireType,v7248);
                (v6723->range = v6724);} 
...
void  at_c_producer(c_producer *v7227)
{ princ_string("->");
  } 

void  generate_tuple_function_c_producer(c_producer *v7227,method *v7237,char *v12719)
{ GC_BIND;
  { list * v11424 = (((v7237->domain->length == 1) && 
        (domain_I_restriction(v7237) == Kernel._void)) ?
      Kernel.nil :
...
OID  create_function_entry_c_producer(c_producer *v7227,lambda *v11422,char *v9226,OID v7237)
{ return (Core.nil->value);} 

OID  update_function_entry_c_producer(c_producer *v7227,ClaireFunction *v9226,list *v11442,ClaireClass *v7243)
{ GC_BIND;
  GC_OBJECT(list,Optimize.OPT->functions)->addFast(_oid_(list::alloc(3,_oid_(v9226),
    _oid_(v11442),
    _oid_(v7243))));
...
{ { char *Result ;
    { int  v7239 = index_list(OBJECT(producer,Generate.PRODUCER->value)->interfaces,_oid_(v1140));
      Result = ((v7239 == 0) ?
        "OID *" :
        string_v((*(OBJECT(producer,Generate.PRODUCER->value)->interfaces))[(v7239+1)]) );
      } 
    return (Result);} 
  } 
...
{ GC_BIND;
  { int  v7239 = index_list(OBJECT(producer,Generate.PRODUCER->value)->interfaces,_oid_(v1140));
    if (v7239 == 0)
     (OBJECT(producer,Generate.PRODUCER->value)->interfaces = GC_OBJECT(list,GC_OBJECT(list,OBJECT(producer,Generate.PRODUCER->value)->interfaces)->addFast(_oid_(v1140)))->addFast(_string_(v7243)));
    else ((*(OBJECT(producer,Generate.PRODUCER->value)->interfaces))[(v7239+1)]=_string_(v7243));
      } 
  GC_UNBIND;} 
...
void  interface_I_c_producer(c_producer *v7227,ClaireClass *v1140)
{ if (v1140 == Kernel._void)
  ...
  else if ((INHERIT(v1140,Kernel._import)) || 
...
void  to_cl_c_producer(c_producer *v7227,OID v7248,ClaireClass *v7243,OID v15308)
{ if (v7243 == Kernel._void)
   { princ_string("_void_(");
  ...
  else if (INHERIT(v7243,Kernel._import))
...
void  to_c_c_producer1(c_producer *v7227,OID v7248,ClaireClass *v7243,OID v15308)
...
void  to_c_c_producer2(c_producer *v7227,ClaireClass *v7243)
...
OID  public_static_c_producer(c_producer *v7227)
{ return (Core.nil->value);} 

void  bool_exp_I_c_producer(c_producer *v7227,OID v1140,OID v15308)
...
void  inherit_exp_c_producer(c_producer *v7227,OID v11032,OID v11033,OID v15308)
...
void  bitvector_exp_c_producer(c_producer *v7227,OID v11032,OID v11033,OID v15308)
...
void  equal_exp_c_producer(c_producer *v7227,OID v11032,ClaireBoolean *v3475,OID v11033,OID v7260)
    ...
    { ClaireBoolean * g0050I;
          ...
          if (v_and == CFALSE) g0050I =CFALSE; 
            ...
            if (v_and == CFALSE) g0050I =CFALSE; 
                ...
                if (INHERIT(OWNER(v11033),Optimize._to_C))
                 v14907 = ((OBJECT(to_C,v11033)->arg == CNULL) ? Kernel.ctrue : Kernel.cfalse);
      ...
      if (g0050I == CTRUE) { princ_string("(equal_string(");
       ...
       { if (equal(_oid_(glb_class(stupid_t_any1(v11032),stupid_t_any1(v11033))),_oid_(Kernel.emptySet)) == CTRUE)
         { warn_void();
          tformat_string("~S = ~S will fail !",2,list::alloc(2,v11032,v11033));
          } 
        princ_string("(");
...
ClaireBoolean * char_exp_ask_c_producer2(c_producer *v7227,OID v7248)
...
void  char_exp_c_producer2(c_producer *v7227,OID v7248,OID v15308)
{ if (INHERIT(OWNER(v7248),Kernel._char))
   { princ_string("'");
    if ((v7248 == _oid_(_char_('\"'))) || 
        ((v7248 == _oid_(_char_('\''))) || 
...
void  c_member_c_producer(c_producer *v7227,OID v1140,ClaireClass *v7243,property *v7248,OID v15308)
...
void  addFast_c_producer(c_producer *v7227)
{ princ_string("->addFast");
  } 

void  cast_I_c_producer(c_producer *v7227,C_cast *v1140,OID v15308)
{ GC_BIND;
  princ_string("((");
  ident_class(v1140->set_arg);
  princ_string(" *) ");
...
void  gc_protection_exp_c_producer(c_producer *v7227,Variable *v7247,ClaireBoolean *v12010,OID v7245,OID v15308)
...
void  bag_expression_c_producer(c_producer *v11165,ClaireClass *v7227,bag *v7236,ClaireType *v7244,OID v15308)
{ if (v7236->length == 0) 
...
void  generate_system_file_string(char *v1140,list *v7236)
{ GC_BIND;
  { ClairePort * v7240 = fopen_string(append_string(GC_STRING(append_string(GC_STRING(_7_string(GC_STRING(Optimize.compiler->source),v1140)),"-s")),GC_STRING(OBJECT(producer,Generate.PRODUCER->value)->extension)),"w");
    ...
    create_loadModules_string(v1140,v7240,v10516);
...
void  create_loadModules_string(char *v1140,ClairePort *v7240,list *v10516)
{ GC_BIND;
  breakline_void();
  { OID gc_local;
    ITERATE(v7248);
  ...
  { OID  v15868;
    if (INHERIT(owner_any(value_string(v1140)),Kernel._module))
     v15868 = value_string(v1140);
...
void  methods_interface_c_producer(c_producer *v7227,ClaireClass *v7248)
...
void  methods_bodies_c_producer(c_producer *v7227,ClaireClass *v7248)
{ GC_RESERVE(6);  // v3.0.55 optim !
  { OID gc_local;
    ITERATE(v7240);
    bag *v7240_support;
