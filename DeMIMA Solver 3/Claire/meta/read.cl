...
keyword?(x:any) : any -> (x % reserved_keyword)
      ...
      (if (n = eof(r)) (next(r), eof)
          ...
          (while (firstc(r) != eof(r) & firstc(r) != 10) next(r),
           ...
           (if (toplevel(r) & (case x (Assign (var(x) % Vardef))))
            ...
            else if (case x (Call (x.selector % Declarations & forall(y in x.args | not(y % Vardef))))) 
 ...
 -> let n := skipc(r) in
      (if (n = eof(r)) (next(r), eof)
              ...
              (if keyword?(x) nextstruct(r, x, e)
               else if (e = none & (case x (Call (x.selector % Declarations)))) x
...
nexte(r:meta_reader) : any
 -> let x := nextexp(r,false) in (if (x % Instruction) LastExpRead := x, x)
...
//
extended_comment!(r:meta_reader,s:string)
  -> (let i := get(s,']'),
          k := substring(s,"//",true),
          m := length(s),
