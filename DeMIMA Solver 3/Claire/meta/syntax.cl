 ...
 -> (*arrow* := false,
     ...
     let table? := (case x (Call (selector(x) = nth &
         ...
         z := nexte(r), w := nexte(r) in
          (if (if table? (w = := | (w = arrow & not(StrictClaire2)))
               else (w = arrow | w = =>)) nil
     ...
     (*arrow* := (y = =>), nextmethod(r,x,any,false,old?,(y = =>)))
    else if (y = := & x % Vardef)
...
nextmethod(r:meta_reader,x:any,y:any,table?:boolean,old?:boolean,inl?:boolean)
  -> (let n := skipc(r),
...
nextinst(r:meta_reader, x:any) : any
 -> (case x
     (Variable (// if StrictClaire2 Serror("Old syntax for global variable ~S", list(x)),
                Defobj(ident = mClaire/pname(x), arg = global_variable,
                      args = list(Call(=, list(range, extract_type(range(x)))),  // v3.1.14
                                  Call(=, list(value, nexte(r)))))),
