...
// recommendation
about() : any
 -> (printf("CLAIRE v3.~S Copyright (C) 1994-2001 Yves Caseau. All Rights Reserved.\n",
            version()),
 ...
 -> (let v:integer := system.debug! in
       (if (v > 0)
           (if (n != 0) set_index(get_stack(v + 3) as integer),
            ...
            if (self.if_write = nil) when m := system.spy! in
...
invert(r:relation,x:any) : bag
 -> (let r2 := get(inverse, r) in
       case r2
        (table let v := r2[x] in (if r2.multivalued? (v as bag) else set(v)),
         property let v := get(r2, x) in
                    (if r2.multivalued? (v as bag) else set(v)),
         any case r
             (property (if r.multivalued? { z in r.domain | x % get(r, z)}
                       else { z in r.domain | get(r, z) = x}),
              table (if r.multivalued? { z in r.domain | x % r[z]}
