...
self_print(self:Defmethod) : void
 -> (printf("~S(~I) : ~I~I ~A ~I ", self.arg.selector,
            ppvariable(self.arg.args), printexp(self.set_arg,false),
                    ...
                    l3 :add
                      (case y
                        (Set
                         ...
                         any
                           extract_pattern(y, (if path path add l1[n])))),
...
self_eval(self:Defarray) : any
 -> (let a := (self.arg as Call).args,
         ...
         e := (let l := cdr(a),
                 ...
                 (if { va in l | occurrence(b, va) > 0} lambda!(l, b)
                  else self.body)),
...
funcall(self:demon,x:any,y:any) -> funcall(self.formula,x,y)
funcall(self:demon,x:any,y:any,z:any) -> funcall(self.formula,x,y,z)
        ...
        if (case c (Call ((c.selector = write | c.selector = nth=) &
           ...
           let R := (c.args[1] as relation), 
             ...
             (if multi?(R) 
              ...
              else tuple(R,list(x,Variable(extract_symbol(y1),R.range),
                                  Variable(gensym(),R.range))))
