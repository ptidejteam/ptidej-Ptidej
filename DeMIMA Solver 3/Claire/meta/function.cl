...
self_print(self:symbol) : any -> printf("~A/~S", module!(self).name,string!(self))
...
//------------------------ FLOAT ---------------------------------------------
self_print(self:float) : any -> function!(princ_float)
...
nth(self:bag,x:integer) : type[member(self)]
...
min(f:method,self:bag) : type[member(self)]     // v3.1.08
 -> (if self let x := self[1] in
                 (for y in self (if funcall(f, y, x) x := y), x)
...
nth=(self:list,x:integer,y:any) : any
  -> (if (x < 0 | x > length(self)) error("[41] nth[~S] out of scope for ~S", x, self) // v3.2.22
...
self_print(self:array) -> printf("array<~S>[~A]",of(self),length(self))
 
//---------------------- CHAR --------------------------------------------
self_print(self:char) : any -> printf("'~A'", self)
