...
breakpoint() : void
 -> (let top := system.Kernel/debug!,
        ...
        *curd* := top,
        *maxd* := top,
        if (get_stack(top) > 0 & system.Kernel/debug! > 0)
           let j := get_stack(top),
               num_args := (get_stack(j + 2) - 1),
               start := (get_stack(j + 3) as integer),
               m := get_stack(j + 1) in
            (printf("break in ~S(~S~I) [q] >", m,
                    get_stack(start),
                    (for i in ((start + 1) .. (start + num_args))
                       printf(",~S", get_stack(i)))),
             let n := 1,
                 m := 1,
...
[dn(x:integer) : void
 -> while (get_stack(*curd*) > 0 & x > 0)
       (*curd* := get_stack(*curd*), *index* :+ 1, x := x - 1) ]
...
[up(x:integer) : void
 ->  if (x > 0)
        let indices := nil,
            ind := *maxd* in
          (while (ind != *curd*)
             (indices := ind cons indices, ind := get_stack(ind)),
...
print_debug_info(index:integer,stack_level:integer,cur_index:integer) : void
 -> (let num_args := ((get_stack(index + 2) as integer) - 1),
         start := (get_stack(index + 3) as integer),
         m := get_stack(index + 1) in
...
Show(n:integer) : any
 -> let i := *curd* in          // i = top of stack
       (while (i > 0 & n > 0)
         let num_args := ((get_stack(i + 2) as integer) - 1),
             start := (get_stack(i + 3) as integer) in
           (printf("[~A - ~A]: p = ~S, narg = ~S \n", start, i,
                   get_stack(i + 1), num_args),
            for j in (0 .. num_args)
              printf("  [~A]:~S \n", j + i, get_stack(j + i)),
            n :- 1,
            i := (get_stack(i) as integer)))         // go to next block
...
claire/block(x:integer) : void
 -> let j := *curd*,
         stack_level := 0 in
      (while (j > 0 & x > 0 & system.Kernel/debug! > 0)
         (let nargs := get_stack(j + 2),
              start := (get_stack(j + 3) as integer),
              z := get_stack(j + 1) as property,
              m := Core/find_which(z.Kernel/definition,
                                    owner(get_stack(start)), start,
                                    start + nargs) in
          (case m (method
              printf("debug[~A] > ~S(~I)\n", *index* + stack_level, m,
                   (if (m.formula % lambda)
                       let n := 0 in
                         for v in closure_build(m.formula)
                           (printf("~S = ~S, ", v, get_stack(start + n)),
                            n :+ 1)
...
claire/PRshow(p:property) : void
 -> let x := p.reified in
      (case x (PRcount printf("~S: ~A calls -> ~A ms\n",p, x.rnum, x.rtime)))
...
PRdependent[p:property] : set[property] := {}
PRdependentOf[p:property] : set[property] := {}
[dependents(self:method) : set[property] //
 -> {p in dependents(self.formula.body) |
     exists(r in p.restrictions | r % method)} as set[property] ]

// this is really cute ....
dependents(self:any) : any
 -> (case self
       (Call add(dependents(self.args),self.selector),
        Instruction let s := {} in
...
claire/PRdepends(p:property,p2:property)
