...
[inlineok?(self:method,s:string) : method
 -> try let p := read,
            l := call(p, s) in
        (write(inline?, self, true), write(formula, self, l))
       ...
       (if (length(p.restrictions) = 5 & uniform(p))
        ...
        if p.dictionary
           (if uniform(x) hashinsert(x)
            else (// disabled in v3.2.08 -> should be back
                  // printf("// ---- note: ~S is not uniform because of ~S \n", p, x),
                  write(dictionary, p, false))),
...
[hashinsert(c:class,x:method) : any
 -> if not(c.dictionary)
        write(dictionary, c, hashlist(29)),
...
[find_which(p:property,n:integer,c:class) : object
 -> (if (length(p.dictionary) != 0) hashget(c, p)
