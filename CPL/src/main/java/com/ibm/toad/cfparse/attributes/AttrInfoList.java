...
//	Reorder by priority Henrique 4/22/2025
	public void reorderByPriority(String[] priority) {
		if (d_attrs == null || d_numAttrs == 0) return;

		AttrInfo[] reordered = new AttrInfo[d_attrs.length];
		int insertIndex = 0;


		for (String target : priority) {
			for (int i = 0; i < d_numAttrs; i++) {
				if (d_attrs[i] != null && d_attrs[i].getName().equals(target)) {
					reordered[insertIndex++] = d_attrs[i];
					d_attrs[i] = null; 
				}
			}
		}
	...
	public AttrInfo add(String name) {
		if (d_attrs == null || d_numAttrs == d_attrs.length) resize();
		int idxName = d_cp.find(ConstantPool.CONSTANT_Utf8, name);
		if (idxName == -1) idxName = d_cp.addUtf8(name);
		...
		AttrInfo attr = switch (name) {
			...
			case "ConstantValue" -> new ConstantValueAttrInfo(d_cp, idxName, d_depth);
			case "Code" -> new CodeAttrInfo(d_cp, idxName, d_depth);
			case "LineNumberTable" -> new LineNumberAttrInfo(d_cp, idxName, d_depth);
			case "LocalVariableTable" -> new LocalVariableAttrInfo(d_cp, idxName, d_depth);
//			Henrique 4/22/2025
...
//			
			...
			case "BootstrapMethods" -> new BootstrapMethodsAttrInfo(d_cp, idxName, d_depth);
	...
	public AttrInfo get(int index) {
		if (index < 0 || index >= d_numAttrs) return null;
		return d_attrs[index];
	}

	public AttrInfo get(String name) {
		for (int i = 0; i < d_numAttrs; i++) {
			if (d_attrs[i].getName().equals(name)) return d_attrs[i];
		}
		return null;
	}
	...
	public void remove(String name) {
		for (int i = 0; i < d_numAttrs; i++) {
			if (d_attrs[i].getName().equals(name)) remove(i);
		}
	}

	public void remove(int index) {
		if (index < 0 || index >= d_numAttrs) return;
...
//				
	...
	public void write(DataOutputStream out) throws IOException {
		...
		for (int i = 0; i < d_numAttrs; i++) d_attrs[i].write(out);
	}

	public void sort(int[] mapping) {
		for (int i = 0; i < d_numAttrs; i++) d_attrs[i].sort(mapping);
	}

	public BitSet uses() {
		BitSet bits = new BitSet(d_cp.length());
		for (int i = 0; i < d_numAttrs; i++) bits.or(d_attrs[i].uses());
		return bits;
	}

	public int size() {
		int size = 2; // short count
		for (int i = 0; i < d_numAttrs; i++) size += d_attrs[i].size();
		return size;
	}
	...
	private void resize() {
		...
		if (d_attrs != null) System.arraycopy(d_attrs, 0, newAttrs, 0, d_numAttrs);
		d_attrs = newAttrs;
	}
