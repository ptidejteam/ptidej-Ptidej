 ...
 * @author tanterij
 */
public class TestPureType extends TestCase {

	private static MicroPatternDetector detector;
	private MicroPatternDetector currentDetector;

	public TestPureType(String arg0) {
		super(arg0);
	}

	protected void setUp() {

		if (TestPureType.detector == null) {
			...
			try {
				codeLevelModel
					.create(new CompleteClassFileCreator(
						new String[] { "../PADL Micro-pattern Analysis/target/test-classes/padl/micropatterns/examples/PureType.class", }));
			}
			catch (Exception e) {
	...
	public void testPureType() {
		Assert.assertEquals(
			"Pure Type",
			1,
			this.currentDetector.getNumberOfPureType());
	}
}
