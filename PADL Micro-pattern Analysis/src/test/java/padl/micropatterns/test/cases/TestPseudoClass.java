 ...
 * @author tanterij
 */
public class TestPseudoClass extends TestCase {

	private static MicroPatternDetector detector;
	private MicroPatternDetector currentDetector;

	public TestPseudoClass(String arg0) {
		super(arg0);
	}

	protected void setUp() {

		if (TestPseudoClass.detector == null) {
			...
			try {
				codeLevelModel
					.create(new CompleteClassFileCreator(
						new String[] { "../PADL Micro-pattern Analysis/target/test-classes/padl/micropatterns/examples/PseudoClass.class", }));
			}
			catch (Exception e) {
	...
	public void testPseudoClass() {
		Assert.assertEquals(
			"Pseudo Class",
			1,
			this.currentDetector.getNumberOfPseudoClass());
	}
}
