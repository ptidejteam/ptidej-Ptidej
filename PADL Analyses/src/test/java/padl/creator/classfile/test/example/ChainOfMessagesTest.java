...
public class ChainOfMessagesTest extends ClassFilePrimitive {
	...
	protected void setUp() throws CreationException,
		...
		if (ChainOfMessagesTest.FirstClassEntities == null
			...
			ChainOfMessagesTest.FirstClassEntities =
				Util.getArrayOfTopLevelEntities(idiomLevelModel);

			ChainOfMessagesTest.Elements =
				Util.getArrayOfElements(ChainOfMessagesTest.FirstClassEntities[3]);
		}
	}
	...
	public void testAssociation0() {
		ClassFilePrimitive.assertAssigable(
			...
			IAssociation.class,
			ChainOfMessagesTest.Elements[1].getClass());
		Assert.assertEquals(
			"Association link visibility",
			Modifier.toString(Modifier.PUBLIC),
			Modifier.toString(ChainOfMessagesTest.Elements[1].getVisibility()));
		Assert.assertEquals(
			"Association link cardinality",
			Constants.CARDINALITY_ONE,
			((IAssociation) ChainOfMessagesTest.Elements[1]).getCardinality());
		Assert
			.assertEquals(
	...
	public void testAssociation1() {
		ClassFilePrimitive.assertAssigable(
			"Association link type",
			IAssociation.class,
			ChainOfMessagesTest.Elements[3].getClass());
		Assert.assertEquals(
			"Association link visibility",
			Modifier.toString(Modifier.PUBLIC),
			Modifier.toString(ChainOfMessagesTest.Elements[3].getVisibility()));
		Assert.assertEquals(
			"Association link cardinality",
			Constants.CARDINALITY_ONE,
			((IAssociation) ChainOfMessagesTest.Elements[3]).getCardinality());
		Assert
			.assertEquals(
				"Association link name",
	...
	public void testAssociation2() {
		ClassFilePrimitive.assertAssigable(
			"Association link type",
			IAssociation.class,
			ChainOfMessagesTest.Elements[4].getClass());
		Assert.assertEquals(
			"Association link visibility",
			Modifier.toString(Modifier.PUBLIC),
			Modifier.toString(ChainOfMessagesTest.Elements[4].getVisibility()));
		Assert.assertEquals(
			"Association link cardinality",
			Constants.CARDINALITY_ONE,
			((IAssociation) ChainOfMessagesTest.Elements[4]).getCardinality());
		Assert
			.assertEquals(
				"Association link name",
