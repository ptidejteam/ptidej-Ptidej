 ...
 * @since	2002/08/15
 */
public class Aggregation1Test extends ClassFilePrimitive {
	private static IElement[] Elements = null;
	private static IFirstClassEntity[] FirstClassEntities = null;

	public Aggregation1Test(final String name) {
		super(name);
	}
	...
	protected void setUp()
		...
		if (Aggregation1Test.FirstClassEntities == null
				|| Aggregation1Test.Elements == null) {
			...
			codeLevelModel.create(new CompleteClassFileCreator(new String[] {
					"../PADL Creator ClassFile/target/test-classes/padl/example/aggregation/Aggregation1.class" }));

			final IIdiomLevelModel idiomLevelModel = (IIdiomLevelModel) new AACRelationshipsAnalysis()
					.invoke(codeLevelModel);

			Aggregation1Test.FirstClassEntities = Util
					.getArrayOfTopLevelEntities(idiomLevelModel);

			Aggregation1Test.Elements = Util
					.getArrayOfElements(Aggregation1Test.FirstClassEntities[2]);
		}
	}

	public void testAddMethod() {
		Assert.assertEquals("Method", "getA",
				Aggregation1Test.Elements[4].getDisplayName());
	}
	...
	public void testConstructor() {
		Assert.assertEquals("Constructor", "Aggregation1",
				Aggregation1Test.Elements[0].getDisplayName());
	}
	...
	public void testContainerAggregation() {
		...
		Assert.assertEquals("ContainerAggregation relationship name",
				...
				Aggregation1Test.Elements[3].getDisplayName());
	...
	public void testField() {
		...
		Assert.assertEquals("Field name", "a",
				Aggregation1Test.Elements[2].getDisplayName());
	}
	...
	public void testGetUseRelationship() {
		...
		Assert.assertEquals("Use relationship name",
				...
				Aggregation1Test.Elements[5].getDisplayName());
		Assert.assertEquals("Use relationship target",
				"padl.example.aggregation.A",
				((IAssociation) Aggregation1Test.Elements[5]).getTargetEntity()
						.getDisplayID());
	}
	...
	public void testMainEntityName() {
		Assert.assertEquals("Entity name",
				...
				Aggregation1Test.FirstClassEntities[2].getDisplayID());
	}

	public void testNumberOfElements() {
		Assert.assertEquals("Number of elements", 8,
				Aggregation1Test.Elements.length);
	}

	public void testNumberOfEntities() {
		Assert.assertEquals("Number of entities", 3,
				Aggregation1Test.FirstClassEntities.length);
	}

	public void testRemoveMethod() {
		Assert.assertEquals("Aggregation", "setA",
				Aggregation1Test.Elements[6].getDisplayName());
	}
