...
public final class RemoveAndIteratorTest extends TestCase {
	...
	public RemoveAndIteratorTest(String name) {
		super(name);
	}
	...
	protected void setUp() throws CloneNotSupportedException {
		...
		this.clonedCompositePattern1 = (IDesignMotifModel) this.compositePattern
				.clone();
	...
	public void testNotConcurrent() {
		try {
			final Iterator iterator = this.clonedCompositePattern1
	...
	public void testEmbeddedNotConcurrent1() {
		int count = 0;
		final Iterator iterator1 = this.compositePattern
				.getIteratorOnConstituents();
		while (iterator1.hasNext()) {
			iterator1.next();

			final Iterator iterator2 = this.compositePattern
					.getIteratorOnConstituents();
	...
	public void testEmbeddedNotConcurrent2() {
		...
		count = 0;
		final Iterator iterator = this.compositePattern
				.getIteratorOnConstituents();
		while (iterator.hasNext()) {
			iterator.next();
			count++;
	...
	public void testConcurrent() {
		final Iterator iterator = this.clonedCompositePattern2
		...
		Assert.assertEquals("Number of entities", 0,
