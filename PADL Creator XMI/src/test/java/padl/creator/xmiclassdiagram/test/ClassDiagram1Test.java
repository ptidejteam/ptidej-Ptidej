...
public class ClassDiagram1Test extends TestCase {
	private IDesignLevelModel model;

	protected void setUp() throws Exception {
		super.setUp();

		this.model = ((MotifFactory) MotifFactory.getInstance())
	...
	public void testAssociation() {
		final IFirstClassEntity gamePresentation = this.model
				.getTopLevelEntityFromID("GamePresentation");
		final IAssociation association = (IAssociation) gamePresentation
				.getConstituentFromID("UMLAssociation.201_>PADL<_1");
		Assert.assertNotNull(association);
		Assert.assertEquals("GameInterface",
				association.getTargetEntity().getDisplayName());
	}

	public void testComposition() {
		final IFirstClassEntity party = this.model
				.getTopLevelEntityFromID("Party");
		final IFirstClassEntity character = this.model
				.getTopLevelEntityFromID("Character");
		final IComposition composition = (IComposition) party
	...
	public void testInheritance() {
		final IFirstClassEntity gamePresentation = this.model
				.getTopLevelEntityFromID("GamePresentation");
		final IFirstClassEntity guiPresentation = this.model
				.getTopLevelEntityFromID("GUIPresentation");
	...
	public void testUseRelationship() {
		final IFirstClassEntity dbManager = this.model
				.getTopLevelEntityFromID("DBManager");
		final IFirstClassEntity iObserver = this.model
				.getTopLevelEntityFromID("i_observer");
		final IUseRelationship useRelationship = (IUseRelationship) dbManager
